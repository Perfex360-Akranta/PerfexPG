...
.padding
{
 ...
 <script type="text/javascript">	
  jQuery(document).ready(function(){	  
	  ...
	  fillComboBox("frmAuditCreation","cmbJhamJhstepid","JHTeamCombo.jhAuditItc?q&templateId="+templateId);
	  ...
	  var fnlnUrl="functionalLoc.jhAuditItc";
	  if(type.trim()=="DMT"){
		  fnlnUrl="functionalLocDMT.jhAuditItc";
	  }
	  if(flId != null && flId != ''){
		 //alert('flId:'+flId);	
		 viewGrid(url,"&s=1&flId="+flId+"&jhamKeyID="+jhamKeyID);
		 loadFunctionalLocation("auditCreationfunLocation",fnlnUrl,"auditCreationfunLocationValues","frmAuditCreation","&q=2&flid="+flId+"&type="+type);
		...
		// alert('else flId:'+flId);		  
		 ...
		 var sectionId = jQuery("#frmAuditCreation input[id='section']").val();
	     ...
	     loadFunctionalLocation("auditCreationfunLocation",fnlnUrl,"auditCreationfunLocationValues","frmAuditCreation",dataStr);
	  }		  
  ...
  jQuery("#btnswitchtoView").click(function(){
   var flid=jQuery("#frmAuditCreation input[id='flid']").val();
  var pillar=getFieldValue("hdnJhamAuditpillar");
  // alert("pillar"+pillar);
   var audittype=getFieldValue("cmbJhamAudittype");
  // alert("audit type is>>>>>>>"+audittype);
   var urll="JhAuditLastThree_input.jhAuditItc";
 LoadPopUp("Last3View",urll+'?&pillar='+pillar+"&flid="+flid+"&audittype="+audittype , true,"80%","70%","20%","30px", " ","Last3Audits",false,false);
  ...
  function btnFilManage_click(){
	    saveForm("frmAuditCreation","jhAuditCreation_save.jhAuditItc?filemanager=true","");
	    /*var documentNo =jQuery("#hdnJhamKeyid").val();		
  ...
  function frmAuditCreation_FuntLocHierarchy_SuccessCallBack(keyIds){
	  ...
	  var type =  getFieldValue("cmbJhamAudittype");
	  if(type.trim()=="DMT") { 
		  	var sectId = jQuery("#frmAuditCreation input[id='section']").val();
		  	var cellId = jQuery("#frmAuditCreation input[id='cell']").val();
  ...
  function btnActionPlan(cellValue, options, rowObject){
	  ...
	  return '<input type="button" id="btnActionplan_'+rowId+'" keyid="'+rowObject[11]+'" name="btnActionplan_'+rowId+'" onclick="event.stopPropagation();openactionplan('+rowId+')"  style="width:50px;  height:15px;"   class="easyui-button" value="..."/>';
  ...
  function txtbox_pointsScored(cellValue, options, rowObject){
	...
	if(cellValue==null || cellValue=='' || cellValue==undefined){
		return "<input id='pointsScored_txtbox"+rowId+"' style='text-align:center;width:70px;height:20px;font-family:arial;' name='pointsScored_txtbox' type='text'  onchange='{fillgrade(\""+rowId + "\");}' onkeypress='return isNumberKey(event);'  />";
		
		
		
	}else{
		return "<input id='pointsScored_txtbox"+rowId+"' maxlength='3' style='text-align:center;width:70px;height:20px;font-family:arial;' name='pointsScored_txtbox' type='text'  onchange='{fillgrade(\""+rowId + "\");}' onkeypress='return isNumberKey(event);' value ='"+cellValue+"' />";
  ...
  function fillgrade(rowId){
	 ...
	 // alert(maxPoints +" maxPoints ");
	  ...
	  if(point.substring(0,6) == '<input')
	  {
		...
		// alert(totalpoint +" totalpointtotalpointtotalpoint");
	 ...
	 // 	alert("pointScored"+pointScored)
	  ...
	  //alert(pointScored +" pointScored "+maxPoints);
  	getStatus();
  }
  ...
  function getJhAudit(){
	...
	if(flid.trim().length>0 && 
		...
		//alert(111);
		 	processAjaxCalls("jhaudit_recall.jhAuditItc","?q=2&flid="+flid+"&templateid="+templateId+"&jhamAuditpillar="+auditpillar+"&jhamAudittype="+audittype+"&stepid="+stepid+"&auditdate="+auditdate+"&auditortype="+auditortype,"jhAuditRecallSuccess","jhAuditRecallError");
	}	
  }
	...
	jQuery("#jhNewAudit").setCell(rowIds[0], '', 'parameterid');	 
	 ...
	 {		 
		...
		//alert(rowId);
		 ...
		 var textboxId = "pointsScored_txtbox" + rowId;
        jQuery("#" + textboxId).on("mousedown keydown click", function(e){
            e.stopPropagation();
        });
 ...
 /* function frmAuditCreationcmbJhamTeam_onSelect(record)
  ...
  function  frmAuditCreationcmbJhamAuditteamid_onSelect(record){
	...
	var templateId=getFieldValue("cmbJhamAuditteamid");
	//alert(templateId);
	  reloadCombo("frmAuditCreation","cmbJhamJhstepid","JHTeamCombo.jhAuditItc?q&templateId="+templateId);
	//alert(record.id);	
	LoadJHParameter(record.id);		
  }
  ...
  function dteJhamAuditdate_onSelect(record){
	  ...
	  if((rowIndex.trim().length>0)){
		  ...
		  if(auditDate == auditDate1)
		  {
			...
			if (result == true)
			{
				 saveForm("frmAuditCreation","jhAuditCreation_save.jhAuditItc","");
  ...
  function getApprovalLevel(){
	  ...
	  processAjaxCalls("fillStatus_recall.jhAuditItc","?q=2&flid="+flId+"&parameter="+parameter+"&auditTeam="+auditTeam+isJhLeader,"minPointsRecallSuccess","minPointsRecallError");
  }
  ...
  function validateMandatory() {
	 ...
	 var sectId = "";
	 sectId = jQuery("#frmAuditCreation input[id='section']").val();
	 var cellId = "";
	 cellId = jQuery("#frmAuditCreation input[id='cell']").val();
...
/* 
 	if(format.test(txtJhamLeadername)==true ||format.test(txtJhamAuditorname)==true){
	...
	if(keyid.length>0){	
	 ...
	 //for(int i=1;i<=jhNewAudit.length;i++)	
	 for(var i=1;i<=1;i++)
	 {
		...
		/*if(pointScored== null || pointScored=='' || pointScored==' '){
			...
			alert('Enter Scored Point for minimum review Points');
			return false;
		}*/
	 }	
  ...
  function convertJHGridToJSONArr(jqGridId){
		...
		jsonArrO=jsonArrO.replace('%','Percentage');
		jsonArrO=jsonArrO.replace('&','And');
		jsonArrO=jsonArrO.replace('&','And');
		jsonArrO=jsonArrO.replace('&','And');
		jsonArrO=jsonArrO.replace('&','And');
		jsonArrO=jsonArrO.replace('&','And');
  ...
  function frmAuditCreation_successsCallback(result)
  {
	 ...
	 if(actplan==true){
		...
		if(actPlanRefDtlId.trim().length>0)
			openActionPlan("divJHAActionPlan",keyid,actPlanRefDocType,flid,actPlanMainTask,actPlanRefDtlId, actPlanRefDate);		  	
  	}	
	 	...
	 	<tr>	 	
			...
			<td >
				<div style="  width : 253px;">
				    ...
				    <div  class="">
				          <textarea onkeyup="restrict(this.value,this.id)" onpaste="restrictPaste(this.value,this.id)"  rows="3"  cols="17" id="txtJhamLeadername" title="Maximum Length is 500" maxlength="500" name="txtJhamLeadername"     style="height : 110px; margin-left: 5px;text-transform: uppercase; width : 214px;"   >${requestScope.jhaTlAuditmst.jhamLeadername}</textarea>
				    </div>
				</div>
			</td>
			<td valign="top">
...
<input style="margin-left:120px;margin-top:-60px;padding-left:0px;" id="btnswitchtoView" class="easyui-button" name="btnswitchtoView"  type="button" value="Audit Score History" style="height:40px;"/>

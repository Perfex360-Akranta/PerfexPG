...
var currentDateCBM ;
  jQuery(document).ready(function(){
	  initialiseForm('frmNewWOGen');
	 // initialiseForm("frmSapDtls");
	  jQuery('#submitForm').val('frmNewWOGen'); // set the id of form to submit.indexOf('&cmbAssmbid=')--&cmbFactid=
	  ...
	  var factId = jQuery("#frmNewWOGen input[id='factory']").val();
	  var sectionId = jQuery("#frmNewWOGen input[id='section']").val();
	  var cellId = jQuery("#frmNewWOGen input[id='cell']").val();
	  var flid = jQuery("#frmNewWOGen input[id='flid']").val();
	 // var machId =getFilterValue(filterString+"&","cmbMchid")  ;
	   var machId = jQuery('#cmbWogenMachineid').combobox('getValue');

	 // alert(filterString.substring(103,111) );
	 fillComboBox("frmNewWOGen","cmbWogenMachineid","machineCombo.commonFilter" );
	//fillComboBox("frmNewWOGen","cmbWogenCostCenter","costCenter.commonFilter");
	fillComboBox("frmNewWOGen","cmbWogenTradeid","combo_pmsdTrade.prv");
	fillComboBox("frmNewWOGen","cmbWogenAssemblyid","assembly.commonFilter");
	 setFieldValue('cmbWogenAssemblyid', assmId,'frmNewWOGen');
	  ...
	  if( ! ( machId != '' && machId != ' ' && machId != null) )
		  machId = jQuery("#frmNewWOGen input[id='machine']").val();

	  var dataStr = "&factId="+factId+"&sectionId="+sectionId+"&cellId="+cellId+"&machId="+machId+"&flid="+flid;	  					
	    loadFunctionalLocation("genwfunLocation","functionalLoc.brdn","genwfunLocationValues","frmNewWOGen",dataStr);
  	...
  	//when coming back from resource planning this will happen
  	 if(whenBack.trim().length>0){
		...
		//processGridnew('completeWO_input.mpce','?q=2&type=PRM&filterString='+jQuery('#hdnfilter_string').val(),"CompWOGrid","","","CompWODBLClick","","CompWOGridComplete");
  	 }
  	  	jQuery.cookie("filterString",filterString);
	  // processGridnew('genWO_input.mpce','?filterString='+filterString ,"GenWOGrid","GenWOPager","","genWODBLClick","","genWOGridComplete");
	   ...
	   /**** cost WO ****/
		jQuery('#btngenCostInfoWo').click(function(){
			  var woMachId = jQuery("#frmNewWOGen input[id='machine']").val();
			  var assmId = getFieldValue("cmbWogenAssemblyid","frmNewWOGen");
			  var tradeId = getFieldValue("cmbWogenTradeid","frmNewWOGen");
   ...
   jQuery("#btnMultipleResp").click(function(){
		 ...
		 var machId = jQuery("#frmNewWOGen input[id='machine']").val();
		...
		if (machId != "")		
			if(already == machId)
				...
				{
					LoadPopUp("loadMultiple", "pmMultipleResp_input.mpce?"+dataString, true,"550px","600px","-1%","7%", "pmMultiResult_successCallBack","Multiple Responsibilty");
...
function getZoneCbmReading(cbmrelated,chkReadingValStr, isCBMAction){
  		...
  		if( cellValue == "Y")
			...
			if(title=="TBM"){
		    	return '<input id="chkformatter" ' +  checked + ' type="checkbox"  onclick="if(this.checked){selectData(\''+id + '\');}else{unselectData(\''+id + '\');}"/>';
	    	     ...
	    	     return '<input id="chkformatter" ' +  checked + ' type="checkbox"  onclick="if(this.checked){dataSelect(\''+id + '\');}else{unselectData(\''+id + '\');}"/>';
  ...
  function divOpenCBM_onClose(){
	  ...
	  var pmstdid = jQuery("#CompWOGridcbm").jqGrid('getCell',rowId,"txtpmstandId");
	  var calenderId = jQuery("#CompWOGridcbm").jqGrid('getCell',rowId,"txtpmCalendarId");
	 ...
	 if(currReading.trim().length>0){
		 var CBMdatStr = jQuery('#CBMWodata').val();
		  ...
		  //alert(CBMdatStr);
		 jQuery("#CompWOGridcbm").jqGrid("setCell", rowId, "cbmreading",currReading);
		 jQuery("#CompWOGridcbm").jqGrid("setCell", rowId, "cbmaction",zoneCondition,{'background-color':ZoneColor});
  ...
  function selectData(rowId){
	  ...
	  var servdate = new Date(srvTime());
	     ...
	     if(title=="CBM"){		   	
	    	 	activity = jQuery("#CompWOGridcbm").jqGrid('getCell',rowId,"frequency");
			    ...
			    jQuery("#txtobservation_"+rowId).removeAttr("disabled");
			  	var sectId = jQuery("#frmNewWOGen input[id='section']").val();			  	
			  	setFormater("CompWOGridcbm","frmNewWOGen","",rowId,"dteObsvTargetDate");			  	
			  	setFormater("CompWOGridcbm","frmNewWOGen","employee.commonFilter?dept=maintenance&sectId="+sectId,rowId,"cmbObsvResponName","hdnObsvResponsibility","180",false, false, false);
				currentDate = new Date(servdate.getFullYear(),servdate.getMonth(),servdate.getDate());
			  	currentDateCBM = servdate;
    		  	jQuery("#CompWOGriddteObsvTargetDate_"+rowId).datebox({onSelect:function(date){
			  	 ...
			  	 cm = jQuery("#CompWOGridcbm").jqGrid("getGridParam", "colModel");  
			 	jQuery("#CompWOGridcbm").setCell(rowId, "hdnchkSel","1");
			  	
				jQuery("#txtactionTaken_"+rowId).val('DONE');
				planVal = jQuery("#CompWOGridcbm").jqGrid('getCell',rowId,cm[10].name);
				jQuery('#txtDuration_'+rowId).val(planVal);
					
			  	jQuery('#selRowId').val(rowId);
			  	
			  	showCBMReadings(rowId);
				setFormater("CompWOGridcbm","frmNewWOGen","",rowId,"txtCbmReading",null,null,null,false,true);
				setFormater("CompWOGridcbm","frmNewWOGen","",rowId,"txtCbmAdjustedReading",null,null,null,false,true);
				setFormater("CompWOGridcbm","frmNewWOGen","",rowId,"dteCbmNextDueDate");
				var freArr = activity.split("-");
				var nextDueDate = getNextDueDate(servdate, freArr[0]);
				jQuery("#CompWOGridcbmdteCbmNextDueDate_"+rowId).datebox({onSelect:function(date){
				  	 var targetDate = new Date(date.getFullYear(), date.getMonth(),date.getDate());
				  	if( targetDate < currentDate ){
				...
				jQuery("#CompWOGridcbmdteCbmNextDueDate_"+rowId).datebox("setValue",nextDueDate);
				
				
		    }
	    else if(title=="TBM"){
		    activity = jQuery("#CompWOGridtbm").jqGrid('getCell',rowId,"frequency");
		    ...
		    enableFields('txtremarks_'+rowId);
		    jQuery("#txtobservation_"+rowId).removeAttr("disabled");
		  	setFormater("CompWOGridtbm","frmNewWOGen","",rowId,"dteObsvTargetDate");
		  	setFormater("CompWOGridtbm","frmNewWOGen","employee.commonFilter?dept=maintenance&sectId="+sectId,rowId,"cmbObsvResponName","hdnObsvResponsibility","180",false, false, false);
		  	jQuery("#CompWOGridtbmdteObsvTargetDate_"+rowId).datebox({onSelect:function(date){
			  	 var targetDate = new Date(date.getFullYear(), date.getMonth(),date.getDate());
			  	if( targetDate < currentDate ){
				  	 alert("Target Date Can not lesser than Current Date");
				  	 jQuery(this).datebox('clear');
		  	...
		  	cm = jQuery("#CompWOGridtbm").jqGrid("getGridParam", "colModel");
			jQuery('#popCBMReadings').hide();		  	
			jQuery("#txtactionTaken_"+rowId).val('DONE');
			planVal = jQuery("#CompWOGridtbm").jqGrid('getCell',rowId,cm[10].name);
			 jQuery('#txtDuration_'+rowId).val(planVal);
			jQuery("#CompWOGridtbm").setCell(rowId, "hdnchkSel","1");
	  	...
	  	activity = jQuery("#CompWOGrid").jqGrid('getCell',rowId,"frequency");
		jQuery("#spnShowActivity").html("Frequency-[Assembly]-Activity : " + activity);  	
	    ...
	    var sectId = jQuery("#frmNewWOGen input[id='section']").val();	  	
	  	setFormater("CompWOGrid","frmNewWOGen","",rowId,"dteObsvTargetDate");	  	
	  	setFormater("CompWOGrid","frmNewWOGen","employee.commonFilter?dept=maintenance&sectId="+sectId,rowId,"cmbObsvResponName","hdnObsvResponsibility","180",false, false, false);
		var servdate = new Date(srvTime());
	  	var currentDate = new Date(servdate.getFullYear(),servdate.getMonth(),servdate.getDate());
	  	currentDateCBM = servdate;
		//var frequncy = jQuery("#CompWOGrid").jqGrid('getCell',rowId,"frequency"); 
	  	jQuery("#CompWOGriddteObsvTargetDate_"+rowId).datebox({onSelect:function(date){
	  	 ...
	  	 cm = jQuery("#CompWOGrid").jqGrid("getGridParam", "colModel");  	 
	  	  jQuery('#selRowId').val(rowId);
	  	  
			jQuery('#txtactionTaken_'+rowId).val('DONE');
				 planVal = jQuery("#CompWOGrid").jqGrid('getCell',rowId,cm[10].name);				
				 jQuery('#txtDuration_'+rowId).val(planVal);
  ...
  function CompWOGridcbm_onBlur(record){
  ...
  function showCBMReadings(rowId){
	  var cbmrelated = jQuery("#CompWOGridcbm").jqGrid('getCell',rowId,"hdnCbmReleated");  
	 // alert(' cbm related' + cbmrelated); 
	  jQuery('#popCBMReadings').hide();
	  jQuery("#popCBMReadingsCont").html("");
	 	var  cbmreadings = cbmrelated.split(",");
	 	//alert('cbmreadings'+ cbmreadings);
		var greenReading = cbmreadings[0].split("#");
		//alert('greenReading'+ greenReading);
		var yellowReading = cbmreadings[1].split("#");
		//alert('yellowReading'+ yellowReading);
		
		var redReading = cbmreadings[2].split("#");
		//alert('redReading'+ redReading);//border: 1px solid black;
		//var htmlStr = "<table border='1px;' style='border:1px;solid ;color:#070302;'><tr class='cbmreadingheader'><th>Zone</th><th>MinReading</th><th>MaxReading</th><th>CorrectiveAction</th></tr>";
		
		var htmlStr = "<table border: 1px solid #070302;'><tr class='cbmreadingheader'><th>Zone</th><th>MinReading</th><th>MaxReading</th><th>CorrectiveAction</th></tr>";
		htmlStr += "<tr border='1' style='border:1px;solid #070302'><td class='cbmreadingtext' style='background-color:"+greenReading[3]+"'>"+greenReading[2]+"</td><td class='cbmreading'>"+greenReading[4]+"</td><td class='cbmreading'>"+greenReading[5]+"</td><td class='cbmreadingtext'>"+greenReading[8]+"</td></tr>";
		htmlStr += "<tr border='1' style='border:1px;solid #070302'><td class='cbmreadingtext' style='background-color:"+yellowReading[3]+"'>"+yellowReading[2]+"</td><td class='cbmreading'>"+yellowReading[4]+"</td><td class='cbmreading'>"+yellowReading[5]+"</td><td class='cbmreadingtext'>"+yellowReading[8]+"</td></tr>";
		htmlStr += "<tr border='1' style='border:1px;solid #070302'><td class='cbmreadingtext' style='background-color:"+redReading[3]+"'>"+redReading[2]+"</td><td class='cbmreading'>"+redReading[4]+"</td><td class='cbmreading'>"+redReading[5]+"</td><td class='cbmreadingtext'>"+redReading[8]+"</td></tr>";
		htmlStr +="</table>";
  ...
  function txtLostFocus(txtid,rowId){
		...
		var chkReadingValStr = jQuery('#CompWOGridcbm'+txtid+"_"+rowId).val();
		if( txtid == "txtCbmReading" ){
			 jQuery("#CompWOGridcbm").jqGrid("setCell", rowId, "colZoneColor","",{'background-color':'white'});
			 jQuery("#CompWOGridcbm").jqGrid("setCell", rowId, "cbmaction","");
			 jQuery("#CompWOGridcbm").jqGrid("setCell", rowId,"hdnCbmMinReading", " ");
			 jQuery("#CompWOGridcbm").jqGrid("setCell", rowId,"hdnCbmMaxReading", "");

			 if(chkReadingValStr.trim() == "")
				return;
			jQuery("#CompWOGridcbmtxtcbmAdjustedReading_"+rowId).val("").removeAttr('disabled').css('background-color', 'white');	
		}
		var cbmrelated = jQuery("#CompWOGridcbm").jqGrid('getCell',rowId,"hdnCbmReleated");
		var  cbmreadings = cbmrelated.split(",");
		var greenReading = cbmreadings[0].split("#");
		...
		if(chkReadingVal >= yellowlwrlmt && chkReadingVal <= yellowuprlmt){
			...
			//alert(chkReadingVal +' chkReadingVal ' + reduprlmt);
			alert("Current Reading Should be between the upper and lower limit values");
			jQuery('#CompWOGridcbm'+txtid+"_"+rowId).val("");
		}
	...
	if( txtid == "txtCbmReading" ){	 
		var nxtDate = jQuery("#CompWOGridcbmdteCbmNextDueDate_"+rowId).datebox("getValue");

		if(nxtDate.trim() == "" ){
			var frequncy = jQuery("#CompWOGridcbm").jqGrid('getCell',rowId,"frequency");
	...
	if(yellowid.length<=0 && redid.length<=0){		
		if(chkReadingVal >= greenlwrlmt && chkReadingVal <= greenuprlmt && greenid == 'green'){
			// greenid   = "green";
			if( txtid == "txtCbmReading" ){
				jQuery("#CompWOGridcbm").jqGrid("setCell", rowId, "cbmaction",greenReading[8]);
				jQuery("#CompWOGridcbm").jqGrid("setCell", rowId, "colZoneColor","",{'background-color':greenC});
				jQuery("#CompWOGridcbm").jqGrid("setCell", rowId,"hdnCbmMinReading", greenlwrlmt);
				jQuery("#CompWOGridcbm").jqGrid("setCell", rowId,"hdnCbmMaxReading", greenuprlmt);
				//jQuery("#CompWOGridcbmtxtCbmAdjustedReading_"+rowId).val("").attr('disabled','disabled').css('background-color', '#D1E2FD');
				jQuery("#CompWOGridcbmdteCbmNextDueDate_"+rowId).datebox("disable");
			}
			//disableField("frmNewWOGen","CompWOGridtxtCbmAdjustedReading_"+rowId);
		...
		//else
			...
			if(chkReadingVal >= yellowlwrlmt && chkReadingVal <= yellowuprlmt){
				...
				if( txtid == "txtCbmReading" ){	
					...
					jQuery("#CompWOGridcbm").jqGrid("setCell", rowId, "colZoneColor"," ",{'background-color':yellowC});
					jQuery("#CompWOGridcbm").jqGrid("setCell", rowId, "cbmaction",yellowReading[8]);
					jQuery("#CompWOGridcbm").jqGrid("setCell", rowId,"hdnCbmMinReading", yellowlwrlmt);
					jQuery("#CompWOGridcbm").jqGrid("setCell", rowId,"hdnCbmMaxReading", yellowuprlmt);
					jQuery("#CompWOGridcbmdteCbmNextDueDate_"+rowId).datebox("enable");
					
				}
				else if( txtid == "txtCbmAdjustedReading" ){
					var znCols = jQuery("tr[id="+rowId+"] > td[aria-describedby=CompWOGridcbm_colZoneColor]").css("background-color");//jQuery("#CompWOGrid").jqGrid("getCell", rowId, "colZoneColor",["background-color"]);
			...
			/*	jQuery('#hdnchkAdjustreading').val('');
				...
				if( txtid == "txtCbmReading" ){	
					jQuery("#CompWOGridcbm").jqGrid("setCell", rowId, "colZoneColor"," ",{'background-color':redC});
					jQuery("#CompWOGridcbm").jqGrid("setCell", rowId, "cbmaction",redReading[8]);
					jQuery("#CompWOGridcbm").jqGrid("setCell", rowId,"hdnCbmMinReading", redlwrlmt);
					jQuery("#CompWOGridcbm").jqGrid("setCell", rowId,"hdnCbmMaxReading", reduprlmt);
					jQuery("#CompWOGridcbmdteCbmNextDueDate_"+rowId).datebox("disable");
					jQuery("#CompWOGridcbmtxtCbmAdjustedReading_"+rowId).val("").attr('disabled','disabled').css('background-color', '#D1E2FD');
				}
				else if( txtid == "txtCbmAdjustedReading" ){
					var znCol = jQuery("tr[id="+rowId+"] > td[aria-describedby=CompWOGridcbm_colZoneColor]").css("background-color");//jQuery("#CompWOGrid").jqGrid("getCell", rowId, "colZoneColor",["background-color"]);
  ...
  function unselectData(rowId){
	  ...
	  removeFormater("CompWOGrid","frmNewWOGen",rowId,"","cmbObsvResponName");
	  removeFormater("CompWOGrid","frmNewWOGen",rowId,"","txtCbmReading");
	  removeFormater("CompWOGrid","frmNewWOGen",rowId,"","txtCbmAdjustedReading");
	  removeFormater("CompWOGrid","frmNewWOGen",rowId,"","dteCbmNextDueDate");
	  ...
	  if(title=="CBM"){
		  ...
		  //removeFormater(gridId,formtType,rowId,value,colName);
		/*  removeFormater("CompWOGridcbm","frmNewWOGen",rowId,"","cmbObsvResponName");
		  removeFormater("CompWOGridcbm","frmNewWOGen",rowId,"","txtCbmReading");
		  removeFormater("CompWOGridcbm","frmNewWOGen",rowId,"","txtCbmAdjustedReading");
		  removeFormater("CompWOGridcbm","frmNewWOGen",rowId,"","dteCbmNextDueDate");
		*/	
	  	jQuery("#CompWOGridcbm").setCell(rowId, "hdnchkSel"," ");
	  	var cm = jQuery("#CompWOGridcbm").jqGrid("getGridParam", "colModel");
			//alert(Object.keys(rowData));
			var j = cm.length;
			 /* 	for(j=7;j<10;j++){
					jQuery("#CompWOGrid").setGridParam({cellEdit:false});
			  ...
			  */	 jQuery('#txtactionTaken_'+rowId).hide();
				 ...
				 jQuery('#txtremarks_'+rowId).hide();
			//jQuery("#CompWOGrid").jqGrid('setCell',rowId,cm[7].name," ",{'color':'#000','background-color':''});
			jQuery("#CompWOGridcbm").jqGrid('setCell',rowId,cm[8].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridcbm").jqGrid('setCell',rowId,cm[9].name," ",{'color':'#000','font-size':'11px','background-color':''});																					
			jQuery("#CompWOGridcbm").jqGrid('setCell',rowId,cm[10].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridcbm").jqGrid('setCell',rowId,cm[12].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridcbm").jqGrid('setCell',rowId,cm[13].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridcbm").jqGrid('setCell',rowId,cm[14].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridcbm").jqGrid('setCell',rowId,cm[15].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridcbm").jqGrid('setCell',rowId,cm[16].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridcbm").jqGrid('setCell',rowId,cm[17].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridcbm").jqGrid('setCell',rowId,cm[18].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridcbm").jqGrid('setCell',rowId,cm[19].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridcbm").jqGrid('setCell',rowId,cm[20].name," -",{'color':'#000','font-size':'11px','background-color':''});
		  }
	  else if(title=="TBM"){

		  //removeFormater(gridId,formtType,rowId,value,colName);
		  removeFormater("CompWOGridtbm","frmNewWOGen",rowId,"","cmbObsvResponName");
		  removeFormater("CompWOGridtbm","frmNewWOGen",rowId,"","txtCbmReading");
		  removeFormater("CompWOGridtbm","frmNewWOGen",rowId,"","txtCbmAdjustedReading");
		  removeFormater("CompWOGridtbm","frmNewWOGen",rowId,"","dteCbmNextDueDate");
			
	  	jQuery("#CompWOGridtbm").setCell(rowId, "hdnchkSel"," ");
	  	var cm = jQuery("#CompWOGridtbm").jqGrid("getGridParam", "colModel");
			//alert(Object.keys(rowData));
			var j = cm.length;
			 /* 	for(j=7;j<10;j++){
					jQuery("#CompWOGrid").setGridParam({cellEdit:false});
			  ...
			  */	 jQuery('#txtactionTaken_'+rowId).hide();
				 ...
				 jQuery('#txtremarks_'+rowId).hide();
			//jQuery("#CompWOGrid").jqGrid('setCell',rowId,cm[7].name," ",{'color':'#000','background-color':''});
			jQuery("#CompWOGridtbm").jqGrid('setCell',rowId,cm[8].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridtbm").jqGrid('setCell',rowId,cm[9].name," ",{'color':'#000','font-size':'11px','background-color':''});																					
			jQuery("#CompWOGridtbm").jqGrid('setCell',rowId,cm[10].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridtbm").jqGrid('setCell',rowId,cm[12].name," ",{'color':'#000','font-size':'11px','background-color':''});

			jQuery("#CompWOGridtbm").jqGrid('setCell',rowId,cm[13].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridtbm").jqGrid('setCell',rowId,cm[14].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridtbm").jqGrid('setCell',rowId,cm[15].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridtbm").jqGrid('setCell',rowId,cm[16].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridtbm").jqGrid('setCell',rowId,cm[17].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridtbm").jqGrid('setCell',rowId,cm[18].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridtbm").jqGrid('setCell',rowId,cm[19].name," ",{'color':'#000','font-size':'11px','background-color':''});
			jQuery("#CompWOGridtbm").jqGrid('setCell',rowId,cm[20].name," -",{'color':'#000','font-size':'11px','background-color':''});
		  }
  }
 ...
 function genWODBLClick(id){
	// alert(id);
	   var mchId = jQuery('#cmbWogenMachineid').combobox('getValue');
		...
		var machineId =mchId;// rowData.machineId;
		var tradeId = 'TDE000003';//rowData.tradeId;
		var mode = jQuery('#hdnmode').val();
	 ...
	 //   var weekNO = filterString1.substring('112');
		...
		filterString += '&cmbAssmbid='+getFieldValue('cmbWogenAssemblyid', 'frmNewWOGen');
		filterString += '&cmbFactid='+ jQuery("#frmNewWOGen input[id='factory']").val();//jQuery('#hdnFctId').val();
		filterString += '&cmbSectid='+  jQuery("#frmNewWOGen input[id='section']").val();//jQuery('#hdnSectId').val();
		filterString += '&cmbCellid='+jQuery("#frmNewWOGen input[id='cell']").val(); //jQuery('#hdnCellId').val();
		filterString += "&cmbTradeid="+tradeId;
		filterString += '&woId='+woId;
		filterString += '&type=PRM';
		//alert(filterString +" filterString ");
		if(jobtype != 'jobtype')
			filterString += '&jobtype='+jobtype;
		else
			filterString += '&jobtype=PRM';
		filterString +='&weekNo='+jQuery("#weekNo").val();
		filterString +='&month='+jQuery('#txtmonthYr').val();
		
		//filterString +=  '&'+weekNO;
		var sectId = jQuery("#frmNewWOGen input[id='section']").val();//jQuery('#hdnSectId').val();
		//jQuery('#hdnfilter_string').val(jQuery('#hdnfilter_string').val()+filterString);
		jQuery('#load1stGrid').css('display','none');
		jQuery('#load2ndGrid').css('display','block');
		jQuery(".spnComplfields").css('display','block').css('margin-left','780px').css('vertical-align','top').css('margin-top','-20px');
		jQuery(".spnComplBtn").css('display','block').css('margin-left','1060px').css('vertical-align','top').css('margin-top','-28px');;
	processGridnew('pmStgTlWostgWo_input.pmstg',filterString,"CompWOGrid","","","CompWODBLClick","","CompWOGridComplete");
	 	
 }
 function frmBDMastercmbbdmsMachineid_onSelect(record)
 {	
		    loadFunctionalLocation("genwfunLocation","functionalLoc.brdn","bdmsfunLocationValues","frmNewWOGen","&machId="+record.id);
		  
 }
 function CompWODBLClick(rowId){

	 		var jobType = jQuery("#CompWOGridcbm").jqGrid('getCell',rowId,"txtjobtype");
	 		if(jobType == "CBM"){
	 			showCBMReadings(rowId);
	 			var activity = jQuery("#CompWOGridcbm").jqGrid('getCell',rowId,"frequency");
	 			jQuery("#spnShowActivity").html("Frequency-[Assembly]-Activity : " + activity);  	
	 		}
			...
			var mach =jQuery("#frmNewWOGen input[id='machine']").val();
...
//		LoadPopUp("loaddetailsPop", "workOrderDetails_input.mpce?"+dataStr, true,"95%","72%","0%","2%", "","Work Order Details");


/*			if(screen.width <= 1024) 
	  	   	  LoadPopUp("loaddetailsPop", "workOrderDetails_input.mpce?"+dataStr, true,"95%","72%","0%","2%", "","Work Order Details");
			else if(screen.width >= 1028 && screen.width < 1366)
			  LoadPopUp("loaddetailsPop", "workOrderDetails_input.mpce?"+dataStr, true,"92%","90%","1%","3%", "","Work Order Details");
			else
		      LoadPopUp("loaddetailsPop", "workOrderDetails_input.mpce?"+dataStr, true,"75%","72%","1%","3%", "","Work Order Details");
	 */ 
	 }
 function CompWOGridComplete(ids){
	...
	jQuery(".zoneColor").each(function(){
	 ...
	 if(mode == 'Y'){
		 disableField('frmNewWOGen','chkformatter');
		 if(modifymode == 'modify'){
		 	...
		 	disableField('frmNewWOGen','cmbbeanWofbCompletedby');
		 //jQuery('input:checkbox[id=chkformatter]').attr('checked',true);
		
	 }
	  
		
		//rowid=jQuery("#CompWOGrid").jqGrid('getDataIDs');
		if(title=="CBM"){
		 rowid = jQuery("#CompWOGridcbm").jqGrid('getDataIDs');
		 var sumval = 0;
			var planVal ="";
			 for(var i=0;i<rowid.length;i++)
			 {
				 if(mode == 'Y' )
				 	jQuery("#CompWOGridcbm").setCell(rowid[i], "hdnchkSel","1");
				 	
				 planVal = jQuery("#CompWOGridcbm").jqGrid('getCell',rowid[i],"plannedduration");	
			 ...
			 rowid = jQuery("#CompWOGridtbm").jqGrid('getDataIDs');
			 var sumval = 0;
				var planVal ="";
				 for(var i=0;i<rowid.length;i++)
				 {
					 if(mode == 'Y' )
					 	jQuery("#CompWOGridtbm").setCell(rowid[i], "hdnchkSel","1");
					 	
					 planVal = jQuery("#CompWOGridtbm").jqGrid('getCell',rowid[i],"plannedduration");	
					 sumval += parseInt(planVal,10);
		...
		else if (rowidcbm!=null){
			noAct=rowidcbm.length;
			alert('cbm');
			}
		else if(rowidtbm!=null){
			noAct=rowidtbm.length;
			alert('tbm');
			}*/
		//var cm = jQuery("#CompWOGrid").jqGrid("getGridParam", "colModel");
...
{
	setFunctionalLocWidth("frmNewWOGen","500px");
...
function  frmWOGencmbWogenAssemblyid_onLoadSuccess(){
	...
	//var assmId = filterString.substring(filterString.indexOf('ASM'),filterString.indexOf('&cmbFactid='));
	 ...
	 ///setFieldValue('cmbWogenAssemblyid', assmId,'frmNewWOGen');
	 //fillComboBox("frmNewWOGen","cmbWogenAssemblyid","assembly.commonFilter");
	 disableField('frmNewWOGen','cmbWogenAssemblyid');
}
function frmWOGen_beforeSubmit(){
	...
	var title=jQuery("#tabtitle").val();
	if(title=="CBM"){
		 gridData = getSelectdRows("CompWOGridcbm","status","hdnchkSel");
		
		}
	else if(title=="TBM"){
		 gridData = getSelectdRows("CompWOGridtbm","status","hdnchkSel");
		
		}
	else{
		 gridData = getSelectdRows("CompWOGrid","status","hdnchkSel");		
...
var completedBy = getFieldValue('cmbbeanWofbCompletedby','frmNewWOGen');
if(completedBy != '' && completedBy != ' ' && completedBy != null){
	if(modifymode == 'modify')
...
function frmWOGen_successsCallback(result){
	var title=jQuery('#tabtitle').val();
	if(title=="CBM"){
		jQuery('#CompWOGridcbm').trigger("reloadGrid");
		}
	if(title=="TBM"){
		jQuery('#CompWOGridtbm').trigger("reloadGrid");
		}
	if(title=="PRM"){
...
function getSelectdRows(jqGridId,checkBoxColName,ckeckForSelColName)
{
	...
	var cm="";
	  var title = jQuery('#tabtitle').val();
		if(title=="CBM"){
			allRows = jQuery("#CompWOGridcbm").jqGrid('getRowData');
			cm = jQuery("#CompWOGridcbm").jqGrid("getGridParam", "colModel");
			
			}
		else if(title=="TBM"){			
			allRows = jQuery("#CompWOGridtbm").jqGrid('getRowData');
			cm = jQuery("#CompWOGridtbm").jqGrid("getGridParam", "colModel");
			
			}
		else if(title=="PRM"){
			allRows = jQuery("#CompWOGrid").jqGrid('getRowData');
			cm = jQuery("#CompWOGrid").jqGrid("getGridParam", "colModel");
						...
						if(colName != 'status' && colName != 'frequency' && colName != 'spares' && colName != 'hdnchkSel' && colName != 'supplier' )
						{
							...
							if( row["txtjobtype"] == "CBM" ) {
								if( colName == "txtCbmReading" && cellValue == ""  ){
									...
									var znCol = jQuery("tr[id="+(i+1)+"] > td[aria-describedby=CompWOGridcbm_colZoneColor]").css("background-color");
									znCol = colorToHex(znCol);
									if( znCol.replace("#",'').toUpperCase() == redC.replace("#",'').toUpperCase()  ){
...
/* SPARE BUTTON IN GRID AND POP UP */
 function sprWOButton(cellvalue, options, rowObject)
	...
	function sprbtnclick(rowId){
		var rowData = jQuery("#CompWOGrid").jqGrid('getRowData',rowId);		
		var pmstdId = rowData.txtpmstandId;
		//processGridnew("sprWOdetails_input.mpce",'&pmstdId='+pmstdId,"sprpopGrid","pager_spr","","");
	...
	jQuery("#btnMultipleResp").click(function(){
		...
		if (machId != "")		
			if(already == machId)
				...
				{
					LoadPopUp("loadMultiple", "pmMultipleResp_input.mpce?"+dataString, true,"550px","600px","-1%","7%", "bdMultiResult_successCallBack","Multiple Responsibilty");
					jQuery("#hdnalready").val(machId);
				}
		else
			alert("Select Equipment");
 ...
 /*****For tabs******/
    ...
    var selr = jQuery('#CompWOGrid').jqGrid('getGridParam','selrow');
    if(selr) alert(selr);
	...
	jQuery("#tabWorkOrder").tabs({ onSelect:function(title){     					
		//jQuery(".tabs-title").bind("click", function(event){
				...
				{	
					...
					//processGridnew('completeWO_input.mpce','?q=2&type=PRM&woId='+woId+'&filterString='+jQuery('#hdnfilter_string').val(),"CompWOGridtbm","","","CompWODBLClick","","CompWOGridComplete");
							
					}
				 else if(title == "CBM" )
					{
					 jQuery("#tabtitle").val('CBM');	
						jQuery("#selectAll").prop('disabled', true);
						var woId=jQuery('#txtWogenwoNo').val();		
												
					//	processGridnew('completeWO_input.mpce','?q=2&type=CBM&woId='+woId+'&filterString='+jQuery('#hdnfilter_string').val(),"CompWOGridcbm","","","CompWODBLClick","","CompWOGridComplete");
					...
					{
					 jQuery("#tabtitle").val('TBM');	
					  jQuery("#selectAll").prop('disabled', true);
										
						var woId=jQuery('#txtWogenwoNo').val();									
					//	processGridnew('completeWO_input.mpce','?q=2&type=TBM&woId='+woId+'&filterString='+jQuery('#hdnfilter_string').val(),"CompWOGridtbm","","","CompWODBLClick","","CompWOGridComplete");
						  // processGridnew('empCostActual_view.crt',"?q=2&formName=empCost&woId="+ woId,"tblEmpCostActGrid","empCostActPager","","tblEmpCostActGrid_dblClick","","costGridLoad","");
					}
   ...
   jQuery('#btnViewClick').click(function(){
	  // alert(123);
	   var mchId = jQuery('#cmbWogenMachineid').combobox('getValue');
	   //alert(mchId +"mchId");
	   processGridnew('pmmobiledevelopmnt_input.mpce','?q=2&mchId='+mchId ,"GenWOGrid","GenWOPager","","genWODBLClick","","genWOGridComplete");

  
	   }); 
   function chkOkFormater(id, options, rowObject)
   {
   	var rowId = options.rowId;	  	
   	return '<input id="Ok_checkbox" name="Ok_checkbox" '+ (rowObject[0]=="1" ? 'checked':'') + ' type="checkbox" ' + 'onclick="if(this.checked){chkboxCheck(\''+rowId + '\');}else{chkboxUnCheck(\''+ rowId +'\')}"/>';
   }
   function chkboxCheck(rowId)
   {

   	jQuery("#CompWOGrid").jqGrid('setCell',rowId,'hdnchkSel','1');	
   }
   function chkboxUnCheck(rowId)
   {

   	jQuery("#CompWOGrid").jqGrid('setCell',rowId,'hdnchkSel','0');
   }
   ...
   function actDrnFormater(cellvalue, options, rowObject){
	     var rowId = options.rowId;
	     var actiontaken  = '<input id="txtActDrn_'+rowId+'"' ;
	       actiontaken  += ' type="text" class="easyui-text"   value="" style="width:200px;text-align:left; " > ';
			return actiontaken;
			}
   function ObsrFormater(cellvalue, options, rowObject){
	     var rowId = options.rowId;
	     var actiontaken  = '<input id="txtOBS_'+rowId+'"' ;
	       actiontaken  += ' type="text" class="easyui-text"   value="" style="width:200px;text-align:left; " > ';
			return actiontaken;
			}
   function actionTakenFormater(cellvalue, options, rowObject){
	     var rowId = options.rowId;
	     var actiontaken  = '<input id="txtActTaken_'+rowId+'"' ;
	       actiontaken  += ' type="text" class="easyui-text"   value="" style="width:200px;text-align:left; " > ';
			return actiontaken;
			}
   
   function frmNewWOGencmbWogenMachineid_onSelect(record)
   { 
   	//alert(record.id+"88");
   	lodFuncLoc("onsel"+record.id);
   	   ...
   	   loadFunctionalLocation("genwfunLocation","functionalLoc.brdn","genwfunLocationValues","frmNewWOGen","&machId="+record.id);
...
//   	alert(9876 +' datStr '+datStr);
   	var compId = getFieldValue('company','frmNewWOGen');	
   	var locnId = getFieldValue('location','frmNewWOGen');	
   	var factId = getFieldValue('factory','frmNewWOGen');	
   	var sectId = getFieldValue('section','frmNewWOGen');	
   	var cellId = getFieldValue('cell','frmNewWOGen');	
   	var machId = getFieldValue('machine','frmNewWOGen');	
   	var flid = getFieldValue('flid','frmNewWOGen');
   	var relTo = '';	
  ...
  // alert(dataStr+" url");	
   loadFunctionalLocation("genwfunLocation","functionalLoc.brdn","toolsfunLocationValues","frmNewWOGen",dataStr);
...
<form name="frmNewWOGen" id="frmNewWOGen">
	...
	<tr>
		<td style="padding-left:20px;">
			...
			<label class="mandatory-lbl" >Equipment </label>
			 <div class=""> 
            <Span>   
             <input id="cmbWogenMachineid" name="cmbWogenMachineid" class="easyui-combobox"  style="width:350px;" value=""  >
           </Span>
          
           <span>
           			<input  type="button" class="easyui-button" id="btnViewClick" name="btnViewClick" value="View"/>		
		...
		<span class="easyui-paddingbfpx spnComplfields" style=" margin-top:-10px;display:none"> 
			<input type="button" value="Back" id="backBtn2nd" class="easyui-button" style="width:150px;" />
		</span>
		</td>
		<tr>
		<td>

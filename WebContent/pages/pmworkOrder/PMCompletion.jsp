...
function getZoneCbmReading(cbmrelated,chkReadingValStr, isCBMAction){
  ...
  function observation_Formatter(cellval, options, rowObject)
  {
	...
	//  if( mode == "Y")
		  disable = "disabled=true";
  		var id = options.rowId;
    	return '<textarea id="txtobservation_'+id +'" ' + disable + '  style="width:145px;height:30px;"  >'+cellval+'</textarea>';
  }
  ...
  function txtLostFocus(txtid,rowId){
	...
	if( txtid == "txtCbmReading" ){	 
		...
		if(nxtDate.trim() == "" ){
			...
			var nextDueDate = getNextDueDate(currentDateCBM, frequncy);//freArr[0]);
 ...
 function genWODBLClick(id){
	   ...
	   alert(mchId+"mchId");
		...
		jQuery(".spnComplfields").css('display','block').css('margin-left','780px').css('vertical-align','top').css('margin-top','-20px');
		jQuery(".spnComplBtn").css('display','block').css('margin-left','1060px').css('vertical-align','top').css('margin-top','-28px');;
		 //  processGridnew('pmStgTlWostg_input.pmstg','?q=2&mchId='+mchId ,"GenWOGrid","","","genWODBLClick","","genWOGridComplete");
...
function frmNewWOGen_FuntLocHierarchy_SuccessCallBack(keyIds)
{
	setFunctionalLocWidth("frmNewWOGen","500px");
	
}
function  frmNewWOGencmbWogenCostCenter_onLoadSuccess(){
	
	   
}
function  frmNewWOGencmbWogenTradeid_onLoadSuccess(){
	
	
}
function  frmNewWOGencmbWogenAssemblyid_onLoadSuccess(){
	 var filterString = jQuery('#hdnfilter_string').val();
...
function frmNewWOGen_beforeSubmit(){
	
	var rowId = jQuery("#selectedDat").val();
	var mach = jQuery("#frmNewWOGen input[id='machine']").val();
	var dataStr = "";
	//alert(mach+ ' before submit' +rowId);
	var gridData = getSelectdRows("CompWOGrid","status","hdnchkSel");
	////alert(gridData+" grid data");
	if( gridData == false)
	return false;
...
//alert(dataStr+" data string");
return dataStr;			
}
function frmNewWOGen_successsCallback(result){
	var title=jQuery('#tabtitle').val();
	if(title=="CBM"){
...
/*	jQuery("#tabWorkOrder").tabs({ onSelect:function(title){     					
		//jQuery(".tabs-title").bind("click", function(event){
				...
				{	
					...
					//	processGridnew('completeWO_input.mpce','?q=2&type=CBM&woId='+woId+'&filterString='+jQuery('#hdnfilter_string').val(),"CompWOGrid","","","CompWODBLClick","","CompWOGridComplete");
				 ...
				 else if(title == "TBM" )
					{
					 jQuery("#tabtitle").val('TBM');	
					  jQuery("#selectAll").prop('disabled', true);
						...
						var woId=jQuery('#txtWogenwoNo').val();									
					//	processGridnew('completeWO_input.mpce','?q=2&type=TBM&woId='+woId+'&filterString='+jQuery('#hdnfilter_string').val(),"CompWOGrid","","","CompWODBLClick","","CompWOGridComplete");
						  // processGridnew('empCostActual_view.crt',"?q=2&formName=empCost&woId="+ woId,"tblEmpCostActGrid","empCostActPager","","tblEmpCostActGrid_dblClick","","costGridLoad","");
					}
   ...
   jQuery('#btnViewClick').click(function(){
	  // alert(123);
	   var mchId = jQuery('#cmbWogenMachineid').combobox('getValue');
	   //alert(mchId +"mchId");
	   processGridnew('pmStgTlWostg_input.pmstg','?q=2&mchId='+mchId ,"GenWOGrid","GenWOPager","","genWODBLClick","","genWOGridComplete");
	   ...
	   /* function SelectFormater(cellValue, options, rowObject)
	   {
			...
			alert(345);
			  ...
			  function selectDataRT(rowId){
				  ...
				  var servdate = new Date(srvTime());
				  	...
				  	activity = jQuery("#CompWOGrid").jqGrid('getCell',rowId,"frequency");
					jQuery("#spnShowActivity").html("[Assembly / Subassembly]-Activity : " + activity);  	
  ...
  /////
   	*/
			  function selectData(rowId){
				  	...
				  	var activity = jQuery("#CompWOGrid").jqGrid('getCell',rowId,"frequency");
				  	alert(activity +" rty "+345);
					jQuery("#spnShowActivity").html("Frequency-[Assembly / Subassembly]-Activity : " + activity);  	
					enableFields('txtDuration_'+rowId);
				    enableFields('txtobservation_'+rowId);
				    enableFields('txtactionTaken_'+rowId);	    
				    jQuery("#txtobservation_"+rowId).removeAttr("disabled");
					...
					//s	setFormater("CompWOGrid","frmNewWOGen","",rowId,"txtCbmReading");//,null,null,null,false,true);
					 	jQuery('#txtactionTaken_'+rowId).val('DONE');
					 	planVal = jQuery("#CompWOGrid").jqGrid('getCell',rowId,cm[6].name);//						
						 jQuery('#txtDuration_'+rowId).val(planVal);
						setFormater("CompWOGrid","frmNewWOGen","",rowId,"txtCbmReading",null,null,null,false,true);
						setFormater("CompWOGrid","frmNewWOGen","",rowId,"txtCbmAdjustedReading",null,null,null,false,true);
						setFormater("CompWOGrid","frmNewWOGen","",rowId,"dteCbmNextDueDate");
						var freArr = activity.split("-");
						alert(freArr +"freArr");
						var nextDueDate = getNextDueDate(servdate,activity);// freArr[0]);
...
//							 jQuery("#CompWOGrid").jqGrid('setCell',rowId,cm[12].name,planVal,{'color':'#000','font-size':'11px','background-color':'#E0DBDB'});
  ...
  ///// 
  /*  function actDurationFormater(cellvalue, options, rowObject){
	     var rowId = options.rowId;
	     var actDuration  = '<input id="txtActDrn_'+rowId+'"' ;
	     actDuration  += ' type="text" class="easyui-text"   value="" style="width:40px;text-align:center; " > ';
			return actDuration;
			}
   function obsFormater(cellvalue, options, rowObject){
	     var rowId = options.rowId;
	     var observation  = '<input id="txtOBS_'+rowId+'"' ;
	     observation  += ' type="text" class="easyui-text"   value="" style="width:180px;text-align:left; " > ';
			return observation;
			}
   function actnTknFormater(cellvalue, options, rowObject){
	     var rowId = options.rowId;
...
function resFormater(cellvalue, options, rowObject){
	     var rowId = options.rowId;
	     var resPonsiblity  = '<input id="txtRes_'+rowId+'"' ;
	     resPonsiblity  += ' type="text" class="easyui-text"   value="" style="width:200px;text-align:left; " > ';
			return resPonsiblity;
			} 
function  resFormater(cellval, options, rowObject){
	  return cellval;
}
function  targetDt_Formatter(cellval, options, rowObject){
	  return cellval;
}
			
function cbmRdngFormater(cellvalue, options, rowObject){
    var rowId = options.rowId;
    var actiontaken  = '<input id="txtActTaken_'+rowId+'"' ;
      actiontaken  += ' type="text" class="easyui-text"   value="" style="width:200px;text-align:left; " > ';
		return actiontaken;
...
<table style="width:100%;padding-left:10px;">
	...
	<tr>
		...
		<span class="easyui-paddingbfpx spnComplfields" style=" margin-top:-10px;display:none"> 
			<input type="button" value="CBM Data" id="btnCbmData" class="easyui-button" style="width:150px;" />

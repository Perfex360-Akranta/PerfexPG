...
jQuery(document).ready(function(){
		...
		/*if((jQuery('#chkpending').is(':checked') == false)&& (jQuery('#chkcompleted').is(':checked') == false))
		{
			...
			if(rowid!=null && rowid!=''&& rowid!=' '&& rowid!=undefined && rowid!='undefined'){
				...
				if(checkForZeroes("dmaicgrd",rowid,5))
					{	
					url = "piechart_chrt.prpo?flag="+flag +"&flid="+conflid; 
					showGraphData(url);
			    }else{ 
				...
				rowid="";
				url = "piechart_chrt.prpo?flag="+flag+"&flid="+flid;   
				showGraphData(url);
 ...
 jQuery('#btnView').click(function(){
     ...
     if( validateFilterSelection(filterString))
	{
     ...
     processGridnew("Dmaic_input.prpo",filterString,"dmaicgrd","dmaicpager",tableCaption,"doubleClickGrid","","dmaic_loadComplete");
     return true;
	}
     });

...
//		ViewGrid(url,"");
		...
		fillComboBox("frmKKProject","cmbWaveNo","comboWave.prpo");
	...
	jQuery('#btnView').click(function(){
		    var wave=jQuery("#cmbWaveNo").combobox("getText");
			var projectname=jQuery("#txtKzpmProjectname").val();
	     processGridnew("kkprojectpriority_input.kkpp","&wave="+wave+"&pronme="+projectname,"KKProject","pager","","","","KKProject_loadComplete");
	     return true;
	});
	
	/*function validateFilterSelection(filter){
		 return true;
	}*/
	...
	function frmKKProject_successsCallback(result)
	{
		...
		if(result.successData.define !="define"){
			...
			navigateToNextForm("projectsprotoview_input.prpo?type=define&isDefineStage=Y&keyid="+jQuery("#hdnMasterKeyid").val() +"&checkList="+checkList,"Project Define",null,forwardData);
		}
	}
	...
	{	
		...
		if(columnName=="ProjectScore"){
			...
			return '<input id="btnDefinestage_'+id+'" name="btnDefinestage_'+id+'" type="button" style="width:76px;height:20px;" value="DefineStage"' + 'onclick="defineStage(\''+id + '\',\''+rowObject[1]+ '\')" class="easyui-button" />';
	      }
	...
	function defineStage(rowid,keyid){
		...
		if(jQuery("#chkdmtcheckbx_"+rowid).is(":checked") == true){
			...
			saveForm("frmKKProject","kkprojectpriority_save.kkpp?"+griddata+"&define=define&rowid="+rowid);
		 ...
		 var masterkeyid = jQuery("#"+jqGridId).jqGrid('getCell',rowid,"MASTERKEYID");
		   if(select=="1"){
			...
			jsonArrO = (jsonArrO != ']'?jsonArrO:"");
			 return encodeURIComponent(jsonArrO);
...
<div><input type="combobox" id="cmbKppmApprovedby" name="cmbKppmApprovedby" class="easyui-combobox"  style="width:160px;/* height:18px; */text-align:right;" value="${requestScope.approved }"/></div>
</td>
 <td style="padding-left: 10px;">
 <div>
...
<div>
<input class="easyui-text"  maxlength="50" style="background-color:cyan;font-weight:bold;width:185px;text-align: center; height:25px;text-align: left;"  id="txtKzpmProjectname"   name="txtKzpmProjectname" value="${requestScope.Project.kzpmProjectname}" />   
</div>
</td>
 <td style="padding-left: 10px;">
<div>
...
<div>
<input type="combobox" id="cmbWaveNo" name="cmbWaveNo" class="easyui-combobox"  style="width:100px;/* height:18px; */text-align:right;" value=""/>
</div>
</td>
<td style="padding-left: 10px;">
<div>
<input class="easyui-button" id="btnView" style="height:25px;width:70px;margin-left:50px;" type="button" value="View"/></div>
</td>
</tr>
</table>

...
function loadComplete()
{
	...
	for(var i=1;i<=allRows.length;i++)
	{
		...
		{
			//alert("j:"+j);
			//alert("cm[j].name1:"+cm[j].name);
			if(cm[j].name=="factoryPillar_checkbox"+(j-1))
			{
				...
				{
					//alert("status:"+status);
					jQuery("#grdKPIProduction").jqGrid('setCell',i,'checkPillarvalue'+j,'1');	
				}
				else
				{
					jQuery("#grdKPIProduction").jqGrid('setCell',i,'checkPillarvalue'+j,'0');
				}
			}
		}
	} 
...
function frmKPIProduction_FuntLocHierarchy_SuccessCallBack(keyIds)
{
	...
	if(keyIds.factId==undefined ||keyIds.factId==''||keyIds.factId==""){
		if(keyIds.sectId==undefined ||keyIds.sectId==''||keyIds.sectId==""){}
		else
			jQuery("#hdnSect").val(sectId);
...
function frmKPIProductioncmbKidlIndicatorid_onSelect(record)
	{
		...
		if(factId==undefined ||factId==''||factId==""){
			...
			{if(sectId==undefined ||sectId==''||sectId==""){dataString+="&factId="+factId;}
			else
				dataString+="&sectId="+sectId;	}

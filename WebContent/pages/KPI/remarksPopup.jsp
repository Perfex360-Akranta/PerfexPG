...
jQuery(document).ready(function(){
	...
	processGridnew(
		    "kpiRemarks_input.kpiActKk",
		    "q=2&kprmDate=" + selectedDate,
		    "KPIremarks", "KPIremarksPager",
		    "Remarks", "", "", "remarksLoadComplete", "", ""
		);
	//processGridnew("kpiRemarks_input.kpiActKk","q=2","KPIremarks", "KPIremarksPager","Remarks","","","remarksLoadComplete","","" );
...
function remarksLoadComplete()
{   
    var bdGridId = jQuery("#KPIremarks").jqGrid('getDataIDs');

    for(var i = 1; i <= bdGridId.length; i++)
    {
        var rowId = bdGridId[i-1];
        var controlId = "txaRemarks_" + rowId;
...
function Remarkslength(rowId)
{
  //alert("rowId"+rowId)	;
//var tval = encodeURIComponent(escape(jQuery('#txaRemarks_'+rowId).val()));
 var tval = jQuery('#txaRemarks_'+rowId).val();
//var remar=encodeURIComponent(escape(jQuery('#txaRemarks_'+rowId).val()));
//alert(remar);
  tlength = tval.length;
 // alert(tlength);
var acclength=parseInt(tlength);
if(acclength>=500)
	  {
	     alert("Maximum Limit is over");
	    jQuery('#txaRemarks_'+rowId).val(jQuery('#txaRemarks_'+rowId).val().substring(0,500));
	   //  alert( jQuery('#txaRemarks_'+rowId).val().length);
...
function txaRemarksFormatter(cellvalue, options, rowObject) {	
	...
	if (cellvalue==' ' || cellvalue==undefined || cellvalue=='undefined ' || cellvalue=='undefined')
		cellvalue="";
	//onkeypress="Remarkslength('+rowId+')"
	//onkeydown="Remarkslength('+rowId+')" 
	var formatStr  = '<textarea onkeyup="restrict(this.value,this.id)" onpaste="restrictPaste(this.value,this.id)" title="Maximum Length is 500" maxlength="500" onkeydown="Remarkslength('+rowId+')" onkeypress="Remarkslength('+rowId+')"  onkeyup="Remarkslength('+rowId+')" id="txaRemarks_'+rowId+'" name="txaRemarks_'+rowId+'" rows=3;cols=60; type="textarea" style="height : 60px;width:245px;"' ;
...
function txtcmbKprmKeyidFormatter(cellvalue, options, rowObject) {
	...
	if (cellvalue==undefined || cellvalue=='undefined ' || cellvalue=='')
		cellvalue="";
	return '<input type="text"  id="txtRemarks_'+options.rowId+'" name="txtRemarks_'+options.rowId+'" '+ 'value=' + cellvalue   +'>';
}
...
function txtChkBoxFormatter(cellvalue, options, rowObject) {
	...
	/*if (cellvalue==undefined || cellvalue=='undefined ' || cellvalue=='')
		cellvalue="";*/
	return '<input  type="checkbox" id="checkbox_'+options.rowId+'" name="checkbox_'+options.rowId+'" '  +'>';
}
...
jQuery('#btnSave').click(function(){
	...
	if(gridData != false ){
		//sriram 19-Nov-2025
		var encoded = encodeURIComponent(gridData);
		saveForm('KPIRemarks','kpiRemarks_save.kpiActKk?&multiplemethods='+encoded+"&action="+action).replace('&','AND');
	}
...
function JqGridToJsonSelectdRowsForGrid(jqGridId,action){
	...
	{	
		...
		// alert(remarks.length);
		 if(remarks.length>500)
			 {
			 popupCommonErrorMsg("Remarks-"+i+"Remarks Exceed the Length! Max Length is 500");
			 flagcheck=true;
			 return false;
	...
	//feb 09
		jsonArrO += '"txtkprmRemarks":"' + unescape(remarks) +'",';
		//jsonArrO += '"txtkprmRemarks":"' +remarks.replace('&','AND')+'",';
	...
	if(flagcheck==true){
		 alert(" Remarks Max length is 500.");
		 return false;
	}

...
<!-- <link rel="stylesheet" type="text/css" media="screen" href="css/grid/jquery-ui-1.8.2.custom.css" />  -->
        ...
        <link rel="stylesheet" type="text/css" href="css/classic-style.css"/>
         <link rel="stylesheet" type="text/css" media="screen" href="css/easyUI_custom.css" /> 
...
<!--  <script type="text/javascript" src="/perfexitc_dev/jquery-easyui-1.11.3/jquery.easyui.min.js"></script>  
 
 <script type="text/javascript" src="/perfexitc_dev/jquery-easyui-1.11.3/locale/easyui-lang-en.js"></script>   -->


<!-- <link rel="stylesheet" type="text/css" media="screen" href="jquery.jqGrid-4.15.5/css/ui.jqgrid.css">
<script src="jquery.jqGrid-4.15.5/js/jquery.jqGrid.min.js"></script> -->
...
<script type="text/javascript" >
	 ...
	 jQuery(document).ready(function(){	
		 ...
		 //console.log("easyUI:"+jQuery.parser.version);
		 //console.log("typeof $.fn.datagrid:"+typeof jQuery.fn.datagrid);
		...
		var isIE = !!window.ActiveXObject || "ActiveXObject" in window;
		 ...
		 if (isFirefox) {
		        // Hide login page
		        $("#login").hide();   // your login container ID

		        // Show test message
		        $("#alertFirefoxDiv").show();    // your test container ID
		    }
			 ...
			 jQuery.ajax({  
		            ...
		            success: function(result){
		            	...
		            	//alert(result.messages);
			            	...
			            	jQuery('#btnLogin').removeAttr('disabled'); // ADDED FOR AFTER LOGIN FAIL
			            	jQuery("#btnForgetPwd").removeAttr("disabled");// ADDED FOR AFTER LOGIN FAIL
		            	...
		            	}else if( result != null && result.concurrentLogin === true ) {
		                    showConcurrentLoginDialog(result.ipAddress, result.machineName);
		 ...
		 function showConcurrentLoginDialog(ip, machine) {

			    
			  jQuery('#concurrentDlg').remove();

			    var html =
			        '<div id="concurrentDlg" style="' +
			            'position:fixed; top:100px; left:45%; transform:translateX(-50%);' +
			            'width:420px; background:#fff; z-index:9999;' +
			            'border:1px solid #99bbe8; border-radius:4px;' +
			            'box-shadow:0 4px 16px rgba(0,0,0,0.4);">' +

			            '<div style="' +
			                'background:linear-gradient(to bottom,#dce9f8,#b8d4f0);' +
			                'padding:8px 14px; font-weight:bold; font-size:13px;' +
			                'color:#1a3a5c; border-bottom:1px solid #99bbe8; border-radius:4px 4px 0 0;">' +
			                'Already Logged In' +
			            '</div>' +

			          
			            '<div style="padding:18px 20px 14px 20px; font-size:13px; color:#333;">' +

			                '<p style="color:#cc0000; font-weight:bold; margin:0 0 14px 0;">' +
			                    'You are already logged in from another System.' +
			                '</p>' +
			                
			                '<input type="button" id="concurrentOk" value="Ok" ' +
	                        'style="width:75px; height:24px; cursor:pointer;' +
	                        'background:#00FF80; color:#333; margin-left:40%; border:1px solid #2a6aad;' +
	                        'border-radius:3px; font-size:13px;"/>' +

			                /* '<table style="width:100%; margin-bottom:16px; border-collapse:collapse;">' +
			                    '<tr>' +
			                        '<td style="padding:4px 0; font-weight:bold; width:140px;">IP Address &nbsp;:</td>' +
			                        '<td style="padding:4px 0;">' + ip + '</td>' +
			                    '</tr>' +
			                    '<tr>' +
			                        '<td style="padding:4px 0; font-weight:bold;">Machine Name:</td>' +
			                        '<td style="padding:4px 0;">' + machine + '</td>' +
			                    '</tr>' +
			                '</table>' +

			                '<p style="margin:0 0 12px 0;">' +
			                    'Do you want to logout that session and login here?' +
			                '</p>' +

			                // Buttons right below the question
			                '<div style="display:flex; gap:10px;">' +
			                    '<input type="button" id="concurrentYes" value="Yes" ' +
			                        'style="width:75px; height:24px; cursor:pointer;' +
			                        'background:#00FF80; color:#333; border:1px solid #2a6aad;' +
			                        'border-radius:3px; font-size:13px;"/>' +
			                    '<input type="button" id="concurrentNo" value="No" ' +
			                        'style="width:75px; height:24px; cursor:pointer;' +
			                        'background:#E8334F; color:#333; border:1px solid #aaa;' +
			                        'border-radius:3px; font-size:13px;"/>' +
			                '</div>' + */
			        ...
			        '</div>' +
			        // Overlay
			        '<div id="concurrentOverlay" style="' +
			            'position:fixed; top:0; left:0; width:100%; height:100%;' +
			            'background:rgba(0,0,0,0.45); z-index:9998;"></div>';

			    jQuery('body').append(html);

			    
			    jQuery('#concurrentYes').on('click', function() {
			        jQuery('#concurrentDlg, #concurrentOverlay').remove();
			        submitLoginWithForce();
			    });

			    jQuery('#concurrentNo').on('click', function() {
			        jQuery('#concurrentDlg, #concurrentOverlay').remove();
			        jQuery('.logLoading').css('display','none');
			        jQuery('.window-mask').css('display','none');
			        jQuery('#btnLogin').css('color','#000');
			    ...
			    jQuery('#concurrentOk').on('click', function() {
			        jQuery('#concurrentDlg, #concurrentOverlay').remove();
			        jQuery('.logLoading').css('display','none');
			        jQuery('.window-mask').css('display','none');
			        jQuery('#btnLogin').css('color','#000');
			...
			function submitLoginWithForce() {
			    jQuery('.logLoading').css('display','block');
			    jQuery.ajax({
			        type: "POST",
			        url: "validate.userLogin",
			        data: jQuery("#login").serialize() + "&confirmForce=Y",
			        dataType: "json",
			        success: function(result) {
			            jQuery('.logLoading').css('display','none');
			            if (result != null && result.tpmuser) {
			                document.location.href = "perfex";
			  ...
			  jQuery.ajax({  
		            ...
		            success: function(result){  
		            	...
		            	{	
			            	...
			            	//document.location.href = "perfex";
			            	var retVal;
			            	if(result.ref == "suceess-jh"){
			            		retVal = confirm("Your password will be reset and mail will sent to JH Leader, are you sure to continue?");
			            	}else{
			            		retVal = confirm("Your password will be reset, are you sure to continue?");
			            	}
...
//***** Unlock the USer *****//
	 ...
	 jQuery("#btnUnLockAct").click(function(){ 
		 alert("In side the unlock butto");
	 
			//jQuery("#btnForgetPwd").one( "click", function(){ 
		 			  jQuery("#btnUnLockAct").css({"background-color": "#FFFFFF"}).css({"color":"#666666"});
		 			  jQuery("#btnUnLockAct").attr("disabled", true).addClass("ui-state-disabled");		 		   	
		 		  // alert("User wants to continue!");
	...
	//****To Show Alert for Version Update*****//
...
//**** END on Un lock User ****//
		...
		function adjustStyle(width) {
		    ...
		    if ( parseInt( width,10) > 1200) {
		        ...
		        jQuery("#itc_logo").attr("src","images/logo/bajaj-logo.png");
	...
	<body   style="padding: 0px;margin: 0px;background-image:url('images/bg1.jpg');background-position: center center;
    background-repeat: no-repeat;
    background-size: cover;scroll 0 0;overflow:hidden;">
	<div id="alertFirefoxDiv" class="alertFirefoxDiv" style="display:none; ">
        <h2 style="color: #d9534f;">This site is not supported in Firefox.</h2><br>
        <h2 style="color: #3cb371;">Please Use Microsoft Edge Browser.</h2>
    </div>
	<form method="post" id="login" onsubmit="btnForgetPwd.disabled = true" style="width: 90%;
    margin: 0 auto;
    padding: 2% 3%;
    ...
    table-layout: fixed;" >	
		...
		<tr style="width:100%;height:20%">
			<td>
				<span style="width: 100%;padding: 1% 3%;box-sizing: border-box;display: flex;justify-content: space-between;align-items: center;">
					<img id="itc_logo" alt="Perfex" src="" style="width: 10%;max-width: 180px;height: 100%;"  />
					 <img id="prefex_logo" alt="Perfex" src="" style="width: 15%;max-width: 280px;height: 100%;"/>
...
<div class="ws_images">
 ...
 <%
    java.util.List<String> slideImgs = (java.util.List<String>) request.getAttribute("loginSlideImgs");
    if (slideImgs != null && !slideImgs.isEmpty()) {
        int i = 0;
        for (String imgName : slideImgs) {
%>
            <li><img src="images/loginslidebajaj/<%= imgName %>" alt="" title="" loading="lazy" style="width:650px;height:431px;" id="wows1<%= i %>"/></li>
<%
            i++;
        }
    }
...
<%
    if (slideImgs != null) {
        int i = 1;
        for (String current : slideImgs) {
%>
    <a href="#" title="">
        <img src="images/home-page/tooltips/img<%= i %>.jpg" alt=""/>
        <%= i %>
    </a>
<%
            i++;
...
<!--			<img src="images/login_slide/example-frame.png" width="700" width="550" alt="Example Frame" id="frame">-->
			...
			<td id="ttTd1" style="width: 32%;
    vertical-align: top;">
			<div id="tt1" style="width: 90%;
    max-width: 400px;
    margin: 0 auto;
    padding: 5%;
    box-sizing: border-box;">
...
<!--                     <option value="VASAI">VASAI</option> -->
            ...
            <span class="lblM">Environment</span>
                ...
                <div style="padding-bottom: 2px;padding-right:10px;width:10%;"> 
                    <select id="cmbEnvironment" name="cmbEnvironment" class="easyui-combobox"  style="width:250px;"  readonly >
...
<!--                <div id="err_txtPswd" class="tpm-errormsg"></div>					-->
               <!--  <div class="p10" style="padding-bottom: 2px; "><span class="lblM">Language</span></div> 
                ...
                <span style="display:none;">
                					<input id="btnUnLockAct" type="button" value="Release Lock" style="height:21px;"  class="easyui-button"  name="btnUnLockAct" />
                
                </span>
                    <div class="logLoading " style=""><label style="float:right;color:#fff;margin-right:-8%">Processing....</label></div>
				</div>
					<!-- Changes on 24Aug23 by Kiran -->
				<!--  	<div> 
					<input id="btnForgetPwd" type="button" value="Forgot Password" style="height:21px;"  class="easyui-button"  name="btnForgetPwd" />

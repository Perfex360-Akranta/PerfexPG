	...
	jQuery(document).ready(function(){//alert("JHCLIT");
		...
		jQuery('#submitForm').val('frmJhClitStd'); 
				...
				if(clismchId != null && clismchId != "")
				{
					...
					navigateToNextForm("jhClit_mcharea.jhclit?q=2&loadContentDivId=jhclitgrid2&preLoadContentDivId=preloadDIVid2&mchId="+clismchId+"&isHidePrevForm=false",frmName);
					...
					if(fromPage=="pcs") {
						var url = "jhClit_frm.jhclit?q=2&stdId=CLI130000004&loadContentDivId=Loadjhclitfrm&preLoadContentDivId=preloadDIVid4";
						LoadForm("jhclitgrid1","preloadDIVid","jhClit_frm.jhclit","dispErr","","jhclitcountgrid_errorCallBack");
					}
					else					
    ...
    {	
		...
		/*if(machineID == "" || machineID == null){
		 ...
		 if( selmchId  == null ||selmchId == "") 
    		...
    		jQuery("#jhclitgrid1").load('jhClit_clickview.jhclit',"?&flid="+flid+"&machineID="+selmchId, function(response, status, xhr) {
			   if (status == "error") {
			 ...
			 <tr>
			 	<td style="width:30%">
			 		...
			 		<div style="margin-left: 60px;" ><label>Equipment</label></div>
		               	...
		               	<input id="cmbClisMachineid" name="cmbClisMachineid" class="easyui-combobox" onkeydown="" style="width:255px;" value="${requestScope.machId}"  >
					</div>
					</div>
			 	</td>
				<td colspan="2" > 
...
<%-- <input type="hidden" id="hdnMachineId" name="hdnMachineId" value="${requestScope.BdmMachineId}"/> --%>
<%-- <input type="hidden" id="hdnJHFlid"  name="hdnJHFlid" value="${requestScope.flid}"/> --%>

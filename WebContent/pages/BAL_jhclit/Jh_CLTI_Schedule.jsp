...
jQuery(document).ready(function(){	
    ...
    jQuery('#abnormalityLink').click(function(){
    	 var refDocId = jQuery('#selRefDocId').val();
    		...
    		var hdnflid = getFilterValue(filtStr,"flid");
    		//alert(filtStr+"  refDocId  "+refDocId+"  selMachineId   "+selMachineId);
    	 navigateToNextForm("Abnormality_input.abnForm?q=2&refDocId="+refDocId+"&filterButton=false"+"&flid="+hdnflid,"Abnormality Identification",null,{"filterString":filtStr});
    });
...
function viewGrid(url,filterString)
{
	if( validateFilterSelection(filterString))
	{
		...
		//alert(url);
		//alert(filterString);
		processGridnew(url,filterString,"list","pager",tableCaption,"doubleclick_grid","","jhCalLoadComplete");
		 jQuery('.ui-jqgrid-labels').toggleClass('white-colr');
		return true;
	}	
}
function doubleclick_grid(id){
	 var rowData = jQuery("#list").jqGrid('getRowData');
	 selId = jQuery("#list").jqGrid('getGridParam','selarrrow');
	 var celValhdn = jQuery('#hdncelvalue').val();
	 var refDocId = jQuery('#selRefDocId').val();
	 var selMachineId = jQuery('#selMachId').val();
	var filtStr=jQuery.cookie("filterString");
	 var celval =jQuery('#celVal_'+id+'_20').text();
	var tickVal= jQuery("#hdnTick").val();
	var UntickVal=jQuery("#hdnUnTick").val();
	var filtStr=jQuery.cookie("filterString");
	var hdnflid = getFilterValue(filtStr,"flid");
	//alert("hdnflid   "+hdnflid+"   filtStr "+filtStr);
	
	 if(celValhdn.trim().length == 1)
		{
...
function jhCalLoadComplete(){
	...
	//hideJqGridRow('list', 2);
	//hideJqGridRow('list', 3);
...
{ 
	...
	//alert(rowObject[rowValue[i]]);
		...
		if(parseInt(rowId) > 3 ){
			formatStr ='<input style="horizontal-align:center;" id="Activity_checkbox'+"_"+rowId+'" name="Activity_checkbox"   type="checkbox"  onclick="if(this.checked){chkboxCheck(\''+rowId + '\',\''+pos+'\');}else{chkboxUnCheck(\''+ rowId +'\',\''+pos+'\')}"/>';
		}
	return formatStr ;
}
function img_tools(id, options, rowObject)
{ 
	var rowId = options.rowId;
	var imgPath= "<span><span>";
	if(parseInt(rowId) > 3 ){
		imgPath="<img style='scroll 0 0;height:22px' src='images/clit_images/EYE.png'>";
		}
	return imgPath;
}
function clrFormatter(cellvalue, options, rowObject) {	
	...
	//for row colors
	 ...
	 var fortickval = rowObject[0];
	// alert("fortickval   " + fortickval);
	var url = jQuery('#hiddenUrl').val();
	...
	chk_box(id,cellvalue);
	 var tickVal ;	
	   ...
	   { 
		...
		else if( cellvalue.trim() != '0' &&  fortickval.charAt(fortickval.length-1)=='0' && rowObject[0]  != "creation" && rowid > 3 ){
			formatStr = '<span id= "celVal_'+id+'_'+position+'"  style="background-color:#C0C0C0;font-weight:bold;font-size:14;padding-bottom:11px;padding-left:10" class="cellWithoutBackground" >'+tickVal+'</span>' ;
			jQuery("#hdnTick").val("1");
			jQuery("#hdnUnTick").val("0");
		...
		else if(cellvalue.trim() != '0' &&  fortickval.charAt(fortickval.length-1)=='1'  && rowObject[0] != "creation" && rowid > 3)//cellvalue.trim() == 'AB' )
		{
			...
			jQuery("#hdnTick").val("0");
			jQuery("#hdnUnTick").val("1");
...
// 				backgroundcolor = backgroundcolor.substr(backgroundcolor.indexOf('background-color:') + 'background-color:'.length, 7);
		...
		if(backgroundcolor =='#C0C0C0' )
		{
			...
			var value = jQuery('#Activity_checkbox'+"_"+(i+1)).attr("value");
			if(value == 1){
				var refdocno = tableDatas[(i)].RefDocno;
				var refdocid= refdocno.split("$"); 
				//alert(i+"  actual  "+tableDatas[(i-1)].RefDocno);
			jsonData  += '{"txtClcaClirefid":"'+refdocid[0]+ '","txtClcaDuration":' +
						 '"'+ tableDatas[(i)].timeinmins + '","txtClcaActualduration":"'+ actual +'","txtClcaPlandate":"'+(  parseInt(colIndex) - 9)  +
...
<input type="hidden" name="hdnFromStandard" id="hdnFromStandard"  value="${requestScope.keyId}" />        
        ...
        <div style="margin-left:50px;color:blue;font-weight:bold;vertical-align:top;">
          ...
          </span></td><td>Plan Not Available</td>
          <td><input type="button" id="btnExcel" class="easyui-button" value="Export Excel" />   </td>
...
<!-- 		<input type="text" id="txtactual" name="txtactual" class="easyui-text"/> -->
		...
		<input type="hidden" id="hdnTick" value="0" />
		<input type="hidden" id="hdnUnTick" value="0" />

...
//processAjaxCalls('cmb_shift.cltishe',dataString,'getShift','getShiftErr');
   // fillComboBox("frmBDMaster","cmbbdmsShiftid","combo_shift.brdn" ); 
...
function  getShift(record)
	{  
  	//reloadCombo("frmclitCal","/","combo_shift.brdn" );
...
jQuery("#btnExcel").click(function(){
	window.open("jhClitcalendar_getExportExcel.cltishe","Excel View");
});
...
//	alert(machId+" machId "+fact +" fact " +sect +" cell "+cellId);
	...
	if(cellId== null || cellId=='' || cellId =='undefined'){
		...
		processGridnew('clitCalendarCompletion_input.cltishe','?q=2&factId='+factId+'&sectionId='+sectionId+'&cellId='+cellId +'&machId='+machId+'&flid='+flid+'&currentDate='+currentDate+'&shiftId='+shift+'',"listGrid","pager_calender","","","","jhCalLoadComplete");//,"","","","jhCalLoadComplete");
...
//alert(currentDate+"........."+date.getDate());
...
function obsFormatter(cellvalue, options, rowObject)
{
	...
	//  if( mode == "Y")
		...
		var id = options.rowId;
	//	alert(id +"txtObservation_txtObservation_");
  	return '<textarea id="txtobservation_'+id +'" ' + disable + '  style="width:180px;height:30px;"  >'+cellvalue+'</textarea>';
}

function  tagFormatter(cellvalue, options, rowObject){
	var disable = "";//"disabled=true";
	var id = options.rowId; 
	return  '<select   id="cmbAbnmTagclassid_'+id +'"' + disable + ' name="cmbAbnmTagclassid" class="easyui-combobox"  style="width:60px;font-size: 12px;height:30px;"><option value=""> </option>   <option title="RED" value="RED">RED</option><option title="WHITE" value="WHITE">WHITE</option></select>';
...
jQuery("#btnSave").click(function(){
  ...
  if(selArray !=null && selArray!=" " && selArray!=""){
	...
	for(var i=0;i<selArray.length;i++)
	{
	  ...
	  // var observation =jQuery("#listGrid").jqGrid('getCell', selrowid,"OBSERVATION");
	   var observationtxt = jQuery('#txtobservation_'+selrowid+'').val();
	   var abnTag=jQuery('#cmbAbnmTagclassid_'+selrowid+'').val();
	//   alert(refId+"  "+observationtxt +"abnTagabnTag" + abnTag);
	 ...
	 // alert(Keyid +" Keyid");
	   var Criteriasplit= Keyid.split(',');
	 	for(var k=0;k<Criteriasplit.length;k++){
		 	...
		 	//jsonArr += '"'+keyvalu + '",';	
		// 	alert("keyvalu"+keyvalu+"Criteriasplit"+Criteriasplit);
		 	jsonArr += '"'+keyvalu + '",';
		 	jsonArr2 += '"'+observationtxt + '",';
		 	jsonArr3 += '"'+abnTag + '",';
 	    }
	 	jsonArr = jsonArr.substring(0,jsonArr.length-1);
		jsonArr += ',';
		jsonArr2 = jsonArr2.substring(0,jsonArr2.length-1);
		jsonArr2 += ',';
		jsonArr3 = jsonArr3.substring(0,jsonArr3.length-1);
		jsonArr3 += ',';
    ...
    var obsevationList=jsonArr2;
    var abnTagClassList =jsonArr3;
   
        if(activityList!=null)
    	{
          //  alert(abnTagClassList+ "  " +obsevationList +"  " +activityList);
        	
        processAjaxCalls("clitCalendar_save.cltishe","q=2?&actvity="+activityList+"&cellId="+cellId+"&mchineId="+mchineId+"&shiftId="+shiftId+"&status="+status+"&observation="+obsevationList+"&abnTagClassList="+abnTagClassList,'getCopyEquipment','getCopyEquipmentError');	
...
jQuery('#btnSendMail').click(function(){
  ...
  if(selArray !=null && selArray!=" " && selArray!=""){
    ...
    var activityList=jsonArr;
        ...
        processAjaxCalls("clitCalendar_save.cltishe","q=2?&actvity="+activityList+"&cellId="+cellId+"&mchineId="+mchineId+"&shiftId="+shiftId+"&status="+status+"&observation="+observation+"&tagClass="+tagClass,'dataSaved','dataSavedError');	
...
function dataSaved(result)
{
	...
	//	alert(cellName+" sectiontsectiont  " + mchineName +" observation "+ observation);
		...
		LoadPopUp("divMailList","employee_mail_input..cltishe?sectionId="+section+"&refid="+refid, true,"40%","300px","200px","50px", null, 'Send Email',false, false,true);
...
<!--  <input type="hidden" name="hdnFromStandard" id="hdnFromStandard"  value="${requestScope.keyId}" />   -->     
	...
	<table  >  
        ...
        <table>       
          ...
          <td> 
            <label  class="mandatory-lbl" id="lblCell" style="margin-left: 25px;">Cell</label>
            </td>
            <td> 
            <label  class="mandatory-lbl" id="lblEquipment" style="margin-left: 25px;">Equipment</label>
            </td>
           <td> 
             <label  class="mandatory-lbl" id="lblShift" style="margin-left: 25px;">Shift</label>
            </td>
            </tr>
            <tr>
             
          <td style="font-size: 12px;  padding-left:25px;">
          ...
          <td style=" padding-left:25px;">
		   			<input type="text" class="easyui-combobox"  id="cmbClitShift" name="cmbClitShift" value="${requestScope.shift}" style="width: 150px;"/>
          </td>
           <td style="padding-left:30px;">
          <input type="button" value="View" class="easyui-button" id="view_Data" style="width:60px;"/>  
          </td>
          
          <td style="padding-left:30px;">
          <input type="button" value="Save" class="easyui-button" id="btnSave" style="width:60px;"/>  
          </td>
...
<table>
          <tr>
          <td style="font-size: 12px; padding-left:25px;">
		  <span class="jh_Calen_planed"  style="padding-top:10px;">
          </span>
          </td>
          <td>Plan Not Available</td>          
		  <td  >
		  <span class="jh_Calen_planned" style="padding-top:10px;"> </span>
          </td><td>Planned</td>
           <td  style="font-size: 12px; ">
           <span class="jh_Calen_completed"  style="padding-top:10px;">
          </span>
          </td><td>Completed</td>
         </tr>
          </table>

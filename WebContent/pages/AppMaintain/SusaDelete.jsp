...
jQuery(document).ready(function(){
	 initialiseForm('frmSusaDelete'); 
	 var url = jQuery('#hiddenUrl').val();
	 formatDateBox('dteFromdate','dd-MMM-yyyy');
	 formatDateBox('dteTodate','dd-MMM-yyyy');
	 fillComboBox("frmSusaDelete","cmbDMT","sectionCombo.commonFilter");
	 fillComboBox("frmSusaDelete","cmbJH","cellCombo.commonFilter");
	 viewGrid(url,"q=2");
	});


function SusaDeleteGrdbtnSusaDelete_onClick(result){	
    var rowid=result.rowId;
	var btnid=result.btnId;
    var SusaKeyid=jQuery("#SusaDeleteGrd").jqGrid('getCell',rowid,"SusnKeyid");
  //  alert(SusaKeyid);
    var r=confirm("Are You Sure To Delete?");
    if(r==true){
    	
        processAjaxCalls("SusaDelete_Delete.appm","SusaKeyid="+SusaKeyid,'update_successCallBack','remove_errorCallBack');    	
    }
    else{
...
function update_successCallBack(result){
	...
	jQuery('#SusaDeleteGrd').trigger("reloadGrid");
}

function viewGrid(url,dataString){  
	processGridnew(url, dataString, "SusaDeleteGrd", "Pager", "", "");
	return true;
}
...
jQuery("#btnview").click(function(){
	...
	processGridnew("SusaDelete_input.appm","?q=2&fromDate="+fromDate+"&toDate="+toDate+"&DMT="+DMT+"&JH="+JH,"SusaDeleteGrd","grdUserPager","","");
});

jQuery("#btnRefresh").click(function(){
	 var url = jQuery('#hiddenUrl').val();
	processGridnew(url,"q=2","SusaDeleteGrd", "Pager", "", "");

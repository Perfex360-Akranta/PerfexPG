...
jQuery(document).ready(function(){
	 initialiseForm('frmSuggestionDelete'); 
	 var url = jQuery('#hiddenUrl').val();
	 formatDateBox('dteFromdate','dd-MMM-yyyy');
	 formatDateBox('dteTodate','dd-MMM-yyyy');
     fillComboBox("frmSuggestionDelete","cmbDMT","sectionCombo.commonFilter");
	 fillComboBox("frmSuggestionDelete","cmbJH","cellCombo.commonFilter");
	 viewGrid(url,"q=2");
	});


function SuggestionDeleteGrdbtnSuggestionDelete_onClick(result){	
    var rowid=result.rowId;
	var btnid=result.btnId;
    var SuggKeyid=jQuery("#SuggestionDeleteGrd").jqGrid('getCell',rowid,"KzbnKeyid");
  //  alert(SuggKeyid);
  var r = confirm("Are You Sure To Delete?");
  if(r==true)
  {
    processAjaxCalls("SuggestionDelete_Delete.appm","SuggKeyid="+SuggKeyid,'update_successCallBack','remove_errorCallBack');
  }
  else{
...
function update_successCallBack(result){
	...
	jQuery('#SuggestionDeleteGrd').trigger("reloadGrid");
}

function viewGrid(url,dataString){  
	processGridnew(url, dataString, "SuggestionDeleteGrd", "Pager", "", "");
	return true;
}
...
jQuery("#btnview").click(function(){
	...
	processGridnew("SuggestionDelete_input.appm","?q=2&fromDate="+fromDate+"&toDate="+toDate+"&DMT="+DMT+"&JH="+JH,"SuggestionDeleteGrd","grdUserPager","","");
});

jQuery("#btnRefresh").click(function(){
	 var url = jQuery('#hiddenUrl').val();
	processGridnew(url,"q=2","SuggestionDeleteGrd", "Pager", "", "");
...
<table id="SuggestionDeleteGrd"></table>
<div id="Pager"></div>
</div>
</form>
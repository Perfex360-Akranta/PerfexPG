...
jQuery(document).ready(function(){	
	...
	//setLoadFormCallBackFrmId("frmJhAuditScoreGraph");
	
	jQuery('#btnGraph').click(function(){
		fnShowJhAuditGraph("LINE");		
	});

	jQuery('#btnBarGraph').click(function(){
		fnShowJhAuditGraph("COLUMN");
	});
});

	function fnShowJhAuditGraph(graphType) {
		var rowid = jQuery("#jhAuditGrid").jqGrid('getGridParam','selrow');
		var flid = "";
		var FirstLevel = 'Y';
		...
		var rowData = jQuery("#jhAuditGrid").jqGrid('getRowData',rowid);
		...
		var url = "chartAuditScoreGraph.JhAuditRpt"+ds;
		
		showGraphData(url);
	}

function jhAuditGrid_loadComplete()
{	
	setDrillDownHeader("CH1-0","jhAuditGrid","KEYFIELD2");
	//setTotalRowCss('jhAuditGrid');
}
...
function viewGrid(url,filterString)
{
	...
	{		
		...
		processGridnew("jhnAuditScoreGraph_input.JhAuditRpt",filterString,"jhAuditGrid","pager",tableCaption,"doubleClickGrid","","jhAuditGrid_loadComplete");		
		return true;
	}
...
function jhAuditGrid_onProcessGridBack(){
	var keyfieldData=jQuery("#hdnFnlnKeyid").val();
	setDrillProcesGridBack("jhAuditGrid","KEYFIELD2",keyfieldData);
	jQuery("#hdnFnlnKeyid").val("");
}			
					
function doubleClickGrid(id){ 
	var keyfieldData=jQuery("#hdnFnlnKeyid").val();
	keyfieldData=setDrillDoubleClick("jhAuditGrid","KEYFIELD2",id,keyfieldData);
	jQuery("#hdnFnlnKeyid").val(keyfieldData);
}
</script>
<form id="frmJhAuditScoreGraph">
...
<table>
    ...
    <td colspan="2">
    <div class="clear"></div>
	<table id="jhAuditGrid" ><tr><td></td></tr></table>
	<div id="pager"></div>

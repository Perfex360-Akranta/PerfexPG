					...
					var tot = rowData.COMPANY3;
					 if( (type=="CUSCOMPNTRPT")){
						  if (jQuery('#chkcrm').is(':checked')==true ){
							var url = "ETplanVScomplinegrapgh.ETPlCompRPT?&flid=&chkline=1&Custype=CRM&tot=TOTAL";
							showGraphData(url);
						 }
					 }
					...
					{			
						...
						var url="";
							    ...
							    if(type.trim().length>0 && (type=="TRADHRPT"||type=="TRADHMEMBERS")){
				                    ...
				                    url = "ETplanVScomplinegrapgh.ETPlCompRPT?&flid=&chkline=1&type="+type+"&tot=TOTAL";
				                }
				                else if(type=="CUSCOMPNTRPT" && type.trim().length>0){
				                	if (jQuery('#chkcomplaintgallery').is(':checked')==true ){
										 url = "ETplanVScomplinegrapgh.ETPlCompRPT?&flid=&chkline=1&Custype=ComplaintGallery&tot=TOTAL";
									 }else if (jQuery('#chkcrm').is(':checked')==true ){
								    	 url = "ETplanVScomplinegrapgh.ETPlCompRPT?&flid=&chkline=1&Custype=CRM&tot=TOTAL";
								    }else
								    {								    	
				                	...
				                	{
				                		 url = "ETplanVScomplinegrapgh.ETPlCompRPT?&flid=&chkline=1";
				                	}
						...
						if(checkForZeroes("PlanVsComGrd",rowid,2)){	
							    ...
							    if(type=="TRADHRPT"||type=="TRADHMEMBERS"){
				                    url = "ETplanVScomplinegrapgh.ETPlCompRPT?&flid="+selId+"&chkline=1&type="+type;
				                }
				                else if(type=="CUSCOMPNTRPT" ){
				                	if (jQuery('#chkcomplaintgallery').is(':checked')==true ){
										 url = "ETplanVScomplinegrapgh.ETPlCompRPT?&flid="+selId+"&chkline=1&Custype=ComplaintGallery";
									 }else if (jQuery('#chkcrm').is(':checked')==true ){
								    	 url = "ETplanVScomplinegrapgh.ETPlCompRPT?&flid="+selId+"&chkline=1&Custype=CRM";
								    }else
								    {								    	
								    	alert('Select Report Type');
								    }
				                ...
				                else
				                	{
				                		 ...
				                		 url = "ETplanVScomplinegrapgh.ETPlCompRPT?&flid="+selId+"&chkline=1";
				                	}
						...
						if(checkForZeroes("PlanVsComGrd",rowid,2)){	
							 /*if(jQuery('#chkcummulative').is(':checked') == true){	
							   var url = "customerCumplaintTrendchart.customerComplaintRpt?rowid=" + selId + "&rownum="+rowid + "&chkcummulative=1";
								 ...
								 //alert("rowid"+rowid);
							   var url = "ETplanVScompchart.ETPlCompRPT?&tot=TOTAL" ;	 
							 //} 
							showGraphData(url);
						}else 
						...
						if(checkForZeroes("PlanVsComGrd",rowid,2)){	
							 /*if(jQuery('#chkcummulative').is(':checked') == true){	
							   var url = "customerCumplaintTrendchart.customerComplaintRpt?rowid=" + selId + "&rownum="+rowid + "&chkcummulative=1";
								 ...
								 //alert("rowid"+rowid);
							   var url = "ETplanVScompchart.ETPlCompRPT?&flid="+selId ;	 
							 //} 
							showGraphData(url);
						}else 
							alert("No Record to View Graph");

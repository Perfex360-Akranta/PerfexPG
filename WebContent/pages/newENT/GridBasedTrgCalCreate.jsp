<script type="text/javascript">
	jQuery(document).ready(
			function() {
				...
				if (flid.length <= 0) {
					loadFunctionalLocation("GrdBsdTrgCallFunctionalLoc", funclocn,"GrdBsdTrgCallFunctionalLoc", "frmGrdBsdTrgCal", "&flid=");
				} else {
					loadFunctionalLocation("GrdBsdTrgCallFunctionalLoc", funclocn,"GrdBsdTrgCallFunctionalLoc", "frmGrdBsdTrgCal", "&flid="
									+ flid);
				...
				fillComboBox("frmGrdBsdTrgCal", "cmbCellid","cellCombo.commonFilter");
...
jQuery("#btnTopicMaster").attr("disabled",true).addClass("ui-state-disabled");
		...
		var		 filterString ="q=2";// "q=2&sectionId="+sectionId+"&cellId="+cellId+"&fromDate="+fromdate+"&todate="+todate+"&uniqPostn="+uniqPostn+"&tradeId="+tradeId;
	...
	function viewGrid(url, filterString) {
		...
		processGridnew(url, filterString, "trgCalendarGrid", "pager", "", "","","onLoadComplete");
	...
	function frmGrdBsdTrgCal_FuntLocHierarchy_SuccessCallBack(keyIds)
	{
			...
			//reloadCombo("frmGrdBsdTrgCal","cmbEtcmTopicid_trgCalendarGrid" + "_" + rowId,"topic_fillcombo.gbtc?&flId="+flid);
			 ...
			 reloadCombo("frmGrdBsdTrgCal", "cmbCellid","cellCombo.commonFilter?&sectionid="+sectid);
...
/*  popupCommonErrorMsg("Select The Location"); 
  ...
  var markReqrd = (function(){
    ...
    try { return $c.combobox("getValue"); } catch(e) { return "N"; }
  })();
  ...
  // --- The two fields that are sometimes blank at Company level ---
  // Prefer the grid cell; if blank, fall back to the hidden(s)/form; if still blank, use "{}"
  var locationId = jQuery("#trgCalendarGrid").jqGrid("getCell", rowid, "txtEtcmLocationId")
                  || jQuery("#hdnlocnid").val()
                  || jQuery("#hdnlocaid").val()
                  || "";

  var sectionId  = jQuery("#trgCalendarGrid").jqGrid("getCell", rowid, "txtEtcmSectionId")
                  || jQuery("#hdnSectionId").val()
                  || jQuery("#frmGrdBsdTrgCal input[id='section']").val()
                  || "{}";   // <-- crucial: never send blank

  // Mode stays as you had it
  var mode = "create";

  // Build the exact same querystring keys you already use (lowercase param names)
  var ds = "&mode="       + encodeURIComponent(mode)
         + "&TraKeyid="   + encodeURIComponent(refDocId || "")
  ...
  // Open the popup exactly as before
  LoadPopUp(
    "divEmployeeAtt",
    "EmpTrainingAtt_input.gbtc?q=2" + ds,
    true, "80%", "80%", "1%", "1%", "", "", false, false
  );
}
...
//---------- altering unique button -- for save -- vignesh 03dec2025 ------------------------------------------------------ // 
...
function trgCalendarGridbtnUniqFactAdd_onClick(result) {
    ...
    var dataStr = "&trgCalId=" + refDocId
                + "&fliId=" + flid
                + "&locaionId=" + locaionId
                + "&sectionId=" + sectionId
                + "&trngType=" + trngType
                + "&trngDuration=" + trngDuration; //+"&trngDate="+trngDate;

    // ⬇⬇⬇ ONLY THIS BLOCK CHANGED TO ENCODE THE URL PROPERLY
    if (refDocId.length == 1 || refDocId == null || refDocId == '') {
        ...
        if (gridval !== "") {
            ...
            // Encode the JSON string so [ { } " , etc. don’t break the URL
            var calendarDetailsParam = encodeURIComponent(gridval);

            var url = "gridBasedCalendar_save.gbtc"
                    + "?calendarDetails=" + calendarDetailsParam
    ...
    // ⬆⬆⬆ END OF CHANGED BLOCK
        ...
        LoadPopUp(
            "divUniquePositionFaculty",
            "trngCalUniquePosition_input.gbtc?q=2" + dataStr,
            true,
            "93%",
            "93%",
            "1%",
...
// 	}
	...
	function trgCalendarGrid_selectRow(rowId) {
		...
		//alert(refDocId +"refDocId");
			...
			var locationId = jQuery("#frmGrdBsdTrgCal input[id='flid']").val();
			// commented by vignesh 
	//		var locationId = jQuery("#frmGrdBsdTrgCal input[id='location']").val();

			var empcnt="0";//jQuery("#hdnEmpCnt").val();
			//alert(flid +" flid" +locationId);
		
			reloadCombo("frmGrdBsdTrgCal","cmbEtcmTopicid_trgCalendarGrid" + "_" + rowId,"topic_fillcombo.gbtc?&flId="+locationId);
					...
					{				
						...
						jQuery('#cmbEtcmCompletedBy_trgCalendarGrid_'+rowid).combobox('setValue','Y');
						 //  jQuery('#cmbEtcmChkCompleted_trgCalendarGrid_'+rowid).combobox('setValue','Y');
						   ...
						   var flid = jQuery("#trgCalendarGrid").jqGrid('getCell', rowid,"txtEtcmKeyid");
						  // alert(flid);
						   fillComboBox("frmGrdBsdTrgCalMod","#cmbEtcmCompletedBy_trgCalendarGrid_"+rowid,"employee.commonFilter?&flid="+flid);
						   jQuery("#cmbEtcmCompletedBy_trgCalendarGrid_"+rowid).combobox('setValue',crntUser);
...
jQuery("#btnSave").click(function () {
    ...
    // This should return a plain JSON string like:
    // [{"dteEtcmCreatedDateTime":"10-Sep-2025", ...}]
    var gridval = getGridSelectArray('trgCalendarGrid') || "[]";

    // ✅ Encode the entire JSON once so the URL is RFC-compliant
    var calendarDetailsParam = encodeURIComponent(gridval);
    ...
    // Your existing helper – no change
    saveForm("frmGrdBsdTrgCal", url);
});
...
// 	});
		...
		LoadPopUp("divTopic","TopicEntryMaster_input.topmst",true,"90%","90%","2%","10px","defectSeverity_Callback","Topic Master","",true,true);
			 ...
			 function trgCalendarGridbtnUniqPos_onClick(result) {
					var rowid = result.rowId;
					var btnid = result.btnId;
					//var refDocId = jQuery('#hdnCalKeyId').val();//
			 ...
			 var Calendarflid = jQuery("#trgCalendarGrid").jqGrid('getCell',rowid,"txtEtcmFlid");
			 var CalendarId=jQuery("#trgCalendarGrid").jqGrid('getCell',rowid,"txtEtcmKeyid");

			 var sectionId =  jQuery("#trgCalendarGrid").jqGrid('getCell',rowid,"txtEtcmSectionId");//jQuery("#frmNewTraCal input[id='section']").val();
			
			 //alert(sectionId);
			// alert(Calendarflid);
			LoadPopUp("DivMultiUniquePosition","MultipleUniqueAdd_input.gbtc?q=2&Calendarflid="+Calendarflid+"&CalendarId="+CalendarId+"&sectionId="+sectionId,true,"800px","490px","4px","4%", "multiSelectOk_Callback","Multi Unique Position Add");

			}
			 
			 function trgCalendarGridbtnMulFaculty_onClick(result) {
					var rowid = result.rowId;
					var btnid = result.btnId;
					//var refDocId = jQuery('#hdnCalKeyId').val();//
			...
			LoadPopUp("DivMultiFaculty","MultipleFaculty_input.gbtc?q=2&Calendarflid="+Calendarflid+"&CalendarId="+CalendarId+"&sectionId="+sectionId,true,"800px","490px","4px","4%", "multiSelectOk_Callback","Multi Unique Position Add");
	...
	<Form id="frmGrdBsdTrgCal" action="">
		<div id="wrapper" style="width: 80%;">
			...
			<div id="dateBtn" style="margin-top: 15px; margin-left: 20px;">
				...
				<span style='margin-left:30px;Display:none;'><label id= '' >JH</label> </span>
				
				   <span style='margin-left:10px; Display:none;'>				
						<input id='cmbCellid' name='cmbCellid' value='${requestScope.entTlTrgCalUnqp.etcuRoleKeyid}' class='easyui-combobox' style='width:200px;' />
			 		</span>
		...
		<input type="hidden" id="hdnLoginUser" name="hdnLoginUser" value="${requestScope.loginUser}"></input> 

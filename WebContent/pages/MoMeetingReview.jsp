...
jQuery(document).ready(function(){	
    ...
    if(frmMode == "view" && frmType !="FIP" || (stage.trim().length==0 && frmType !="FIP" && frmType !="JH" && frmType !="Dmt" && frmType !="Pillar" && frmType !="Production" && frmType !="Others")){
		jQuery('#btnNew').hide();
	}else if(frmMode == "view" && frmType=="FIP" && stage.trim().length==0)
		jQuery('#btnNew').hide();
	...
	if( filterStr != null && filterStr.length > 0 )
	{
		...
		processGridnew(url,dataString+'&momRefDocId='+jQuery("#hdnMomRefDocId").val()+'&momRefDocType='+jQuery("#hdnMomRefDocType").val(),"list","pager",tableCaption,"docDoubleClick");
	}
	else{
		setLoadFormCallBackFrmId("frmMomMaingrid");
		invokeAfterLoadFormCallBack();
		//viewGrid("MinMeetingRvw_input.mom","q=2"+'&momRefDocId='+jQuery("#hdnMomRefDocId").val()+'&momRefDocType='+jQuery("#hdnMomRefDocType").val());
	}
...
function  getMOMNwShift(record)
{
	...
	function viewGrid(url,dataString)
	{
		...
		{   
			...
			processGridnew("MinMeetingRvw_input.mom",dataString,"list","pager","","docDoubleClick");
		    return true;
		}
	...
	function frmMomMaingrid_afterLoadCallBack(){
		console.log("frmMomMaingrid_afterLoadCallBack called");
		toggleCommonFilter(); 
...
{	
	...
	//alert(" Inside :: "+jQuery('#hdnDMT').val());
	var meetingType = jQuery('#list tr#' + id + ' td[aria-describedby="list_MEETINGTYPE"]').text().trim();
	console.log("Meeting Type:", meetingType);
	if(meetingType == 'JH')
	{
		momType='J';
	}
	else if(meetingType == 'DMT')
	{
		meetingType='D';
		console.log(meetingType +" enterednksdjd");
	}
	else if (meetingType.startsWith("Pillar")) {
		meetingType='P';
		console.log(meetingType +" enterednksdjd");
	}
...
<table style="margin-top:-20px;">
 <tr>
	...
	<td>
    <div style="padding-left:4px;"><input class="easyui-button" type="button" id="btnNew" name="btnNEW" value="New Entry" style="display:none;">

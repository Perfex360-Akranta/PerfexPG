...
jQuery(document).ready(function(){
	...
	fillComboBox("frmuniqueposition","cmbuniqueposition","roleMst.commonFilter");  
...
function frmuniqueposition_successsCallback(result){
	...
	//processGridnew("UniquePositionSelectd_input.topi","?q=2&flid="+flid+"&upid="+roleKey,"selectedRoleGrd","rolepageremp","","uniqueempdoubleclick","","upempload_complete");
}
function  frmuniquepositioncmbuniqueposition_onSelect(record)
{ 
	jQuery('#txtRoleName').val(record.text);
...
{  
	...
	viewGrid("&upid="+getFieldValue("cmbuniqueposition")+"&flid="+keyIds.flId);
	reloadCombo("frmuniqueposition","cmbuniqueposition","roleMst.commonFilter?&flid="+keyIds.flid);
}
...
function viewGrid(filterStr){
	var upid = getFieldValue("cmbuniqueposition");	//alert(comboid);
...
function empload_complete(ids){ 
	...
	var comboid = getFieldValue("cmbuniqueposition");	//alert(comboid);
	if(comboid.trim().length==0)
		comboid = jQuery('#hdnroleKeyid').val();
	 for(var i=0;i<row.length;i++)
	 {
		var roleid = jQuery("#upEmployeegrid").jqGrid('getCell',row[i],"empm_roleid");
		...
		<table>
			<tr>
				<td colspan='3'>
					<div  id="frmuniquepositionFuntKeyIds">
						...
						<input type="hidden" id="elementId" name="hdnElementid" value=""  ></input>
			...
			<tr>
				<td valign="top"  style="width:32%;">
							...
							<div>
								<input class="easyui-combobox" id="cmbuniqueposition" name="cmbuniqueposition"  value="${requestScope.gentlrolemst.roleKeyid}" style=" width : 200px;"  />
							</div>
				</td>				
				...
				<td style="width:32%;">
					<div class="easyui-paddingbfpx">
							...
							<div>
								<input class="easyui-text" id="txtRoleName" name="txtRoleName"  value="${requestScope.gentlrolemst.roleName}" style=" width : 200px;text-transform: uppercase"  />
							</div>
				</td>
				<td>
					<div class="easyui-paddingbfpx">
							...
							<div>
								<input class="easyui-text" id="txtRoleCode" name="txtRoleCode"  value="${requestScope.gentlrolemst.roleCode}" style=" width : 200px;text-transform: uppercase"  />
							</div>
				</td>
			</tr>
	</table>

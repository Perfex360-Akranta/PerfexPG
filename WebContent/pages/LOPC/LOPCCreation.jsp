...
/* function frmLOPCCreation_beforeSubmit(){    	
   ...
   /*  
	...
	function frmLOPCCreation_successsCallback(result){
	    ...
	    // Clear comboboxes explicitly
	    jQuery("#cmbLoemLocacategoryid").combobox('clear');
	    jQuery("#cmbLoemIdentifiedby").combobox('clear');
	    jQuery("#cmbLoemEmployeeid").combobox('clear');
	    jQuery("#cmbNmrnCompletedby").combobox('clear');
	    jQuery("#cmbNmrnApprovedby").combobox('clear');
	    jQuery("#cmbnmrnResponsibility").combobox('clear');
	    jQuery("#cmbNmrnDeptid").combobox('clear');
	    jQuery("#cmbNmrnProbablerecrate").combobox('clear');
	    jQuery("#cmbNmrnStatus").combobox('clear');

	    // Clear text fields
	    jQuery("#txtLoemLopcdesc").val('');
	    jQuery("#txtNmrnDescnearmiss").val('');
	    jQuery("#txtNmrnActionrecommended").val('');
	    jQuery("#txtNmrnRemarks").val('');
	    jQuery("#txtNmrnInvestigation").val('');

	    // Clear hidden fields
	    jQuery("#hdnLopcID").val('');
	    jQuery("#hdnFilemngr").val('');
	    jQuery("#hdnWwblKey").val('');
	    jQuery("#hdnStatus").val('');
	    jQuery("#hdnFnlid").val('');

	    // Clear hidden functional location keys
	    jQuery("#frmLOPCCreation input[id='flid']").val('');
	    jQuery("#frmLOPCCreation input[id='cell']").val('');
	    jQuery("#frmLOPCCreation input[id='section']").val('');
	    jQuery("#frmLOPCCreation input[id='machine']").val('');

	    // Reset checkboxes
	    jQuery("#chkkzbnOthers").prop('checked', false).val('N');
	    jQuery("#chkkzbnPrepareOthers").prop('checked', false).val('N');
	    ...
	    // Reload functional location (reset to default - no flid)
	    loadFunctionalLocation(
	        "LopcFunLocation",
	        "functionalLoc.lopc",
	        "LopcFunLocationValues",
	        "frmLOPCCreation",
	        ""
	    );

	    // Reset document number
	    jQuery("#txtdocuno").val('');

	    // File manager popup if needed
	    if(result.successData.openfilemgr == "true"){
	        var documentNo = result.successData.keyId;
	        if(documentNo != undefined){
	            fileManagerPopUp(documentNo, "NM", "", "", "", "create");
	        }
	    }

	    // Reload grid if present
	    if(jQuery("#lopcModifyGrd").length > 0) {
	        jQuery("#lopcModifyGrd").trigger('reloadGrid');
	    }
	}

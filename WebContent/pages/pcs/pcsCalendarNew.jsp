<script type="text/javascript">
	...
	jQuery(document).ready(function(){
		...
		fillComboBox("frmPcsCalendar","cmbPCSMachineid","machineCombo.commonFilter?q=2&pcsEnabled=Y&factId="+factId  );
		...
		if (cellId != null)
			loadFunctionalLocation("pcsCalendarfunLocation","functionalLoc.commonFilter","pcsCalendarfunLocationnValues","frmPcsCalendar","&cellId="+cellId);
		else
			loadFunctionalLocation("pcsCalendarfunLocation","functionalLoc.commonFilter","pcsCalendarfunLocationValues","frmPcsCalendar",dataStr);
	...
	/*	if (!( sectionId == ""  || sectionId == ""))		
		...
		var factId = "";//jQuery("#frmPcsCalendar input[id='factory']").val();
		var dataString = '?q=2&factId='+factId+'&sectId='+jQuery("#frmPcsCalendar input[id='section']").val();
	...
	*/
	...
	//	fillComboBox("frmPcsCalendar","cmbShiftid","shift.commonFilter"+"?q=2&factId="+factId+"&frmRfilter=yes");
		...
		reloadCombo("frmPcsCalendar","cmbPCSMachineid","machineCombo.commonFilter"+dataStr  );

		if (keyIds.cellId!="null")
		 	setFieldValue('cmbCellid',keyIds.cellId);
		  
		if (keyIds.machId!="null")
		  	setFieldValue('cmbPCSMachineid',keyIds.machId);
	...
	function pcsEnableforShift(result)
	 {		
		...
		if(result.isPcsEnabled == 'Y') {
			...
			var sectId = jQuery("#frmPcsCalendar input[id='section']").val();
			var factId =  "";// jQuery("#frmPcsCalendar input[id='factory']").val();
			var mchId = jQuery('#cmbPCSMachineid').combobox("getValue");
			var mode = jQuery('#hdnMode').val();
			//alert(mode);
			dataStr+= "&mchId="+mchId+"&sectId="+sectId+"&factId="+factId+"&mode="+mode+"&formAuto=false";
			//alert(dataStr);							
	...
	//function getShiftSuccess(result) {
		...
		var mode = jQuery('#hdnMode').val();
		var formLock = "";		
		dataStr+= "&mchId="+mchId+"&cellId="+cellId+"&sectId="+sectId+"&factId="+factId+"&mode="+mode+"&formAuto=false";
		//alert(dataStr);
	...
	function frmPcsCalendarcmbCellid_onSelect(record) 	{
		//alert(record.id);
		jQuery('#cmbPCSMachineid').combobox('clear');
		loadFunctionalLocation("pcsCalendarfunLocation","functionalLoc.commonFilter","pcsCalendarfunLocationnValues","frmPcsCalendar","&cellId="+record.id);
		jQuery("#monthGrid").clearGridData();
		//fnViewPcs();
	}
	
	function frmPcsCalendarcmbPCSMachineid_onSelect(record) 	{
		//alert(record.id);
	  loadFunctionalLocation("pcsCalendarfunLocation","functionalLoc.commonFilter","pcsCalendarfunLocationnValues","frmPcsCalendar","&machId="+record.id);
	  jQuery("#monthGrid").clearGridData();
	  //fnViewPcs();
...
<input type="hidden" id="hdnAllowDays" name="hdnAllowDays"/>
	...
	<div style="">
		...
		<label style="padding-left: 183px;" >Equipment</label>
		<label class="mandatory-lbl" style="padding-left: 151px;" >Date</label>
		<label style="padding-left: 70px;" >Shift</label>
	</div>
    <div class="easyui-paddingbfpx" style="" >
		...
		<span style="padding-left: 1%;">
	    	<input type="text" id="cmbPCSMachineid" name="cmbPCSMachineid" class="easyui-combobox" value="${requestScope.mchId}" style="width: 200px;"/>
	    </span>	    								
		<span style="padding-left: 1%;">
    	...
    	<span style="padding-left: 1%;">
	    	<input type="text" class="easyui-combobox" id="cmbShiftid" name="cmbShiftid" readonly="readonly"  style="width: 80px;"  value="${requestScope.shift}"/>	    	
	    </span>	 
    </div>
     <div class="easyui-paddingbfpx" style="margin-top: 4px" >
		...
		<span style="padding-left: 0.5%;">
		    <input type="button" id="btnViewPCS" name="btnViewPCS" class="easyui-button"  value="Production Entry" align="middle" style="width:17%\9;"/>
		</span>	
    </div>
...
<!--					<div id="monthPager" style="float: center;">	</div> -->				
	...
	<td valign="middle" align="left" style="margin-left: 1%;">
			...
			<div>
				<input type="text" value="PCS Completed" disabled="disabled" style="margin-top:12px; width:150px;height:25px;border:1px solid rgba(51, 153, 255, 1) ;color:black;background-color:#c0ffc0;font-weight:bold;text-align:center; " />
			</div>
			<div>
				<input type="text" value="QA Completed" disabled="disabled" style="margin-top:12px;width:150px;height:25px;border:1px solid rgba(51, 153, 255, 1) ;color:black;background-color:#00a600;font-weight:bold;text-align:center; " />
			</div>			
			<div>

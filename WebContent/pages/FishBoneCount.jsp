...
function viewGrid(url,filterString)
{
	if( validateFilterSelection(filterString))
	{
		...
		var tableCaption = "DrillDown FishBone Count Report";
...
//doubleclick for drilldown fixed -- by vignesh   - 12dec2025
...
function doubleClickGrid(id) {
	...
	var rowId=jQuery('#impVscomp').jqGrid("getGridParam", 'selrow');	
	//alert("rowId " + rowId);
	var celValue = jQuery('#impVscomp').jqGrid ('getCell', rowId, 'keyid2'); //Company3
   // alert("celValue " + celValue);
	
 // get 3 chars after L=
   // var m = celValue.match(/L=([A-Za-z0-9]{3})/);
  //  var drillCaption = (m && m[1]) ? m[1] : "";   // "CMP"
    
 // Extract value after L= and before #
    var m = celValue.match(/L=([^#]+)/);
    var drillCaption = m ? m[1] : "";

   // alert("drillCaption = " + drillCaption);
   
// 	var filterString = '?q=2&drillFlag=F';		
// 	var type=jQuery('#hdntype').val();
// //	alert("type " + type);
// 	filterString += '&drillFlag=f&firstClick=N';
// 	filterString += '&type='+type;
// 	var tableCaption = "DrillDown FishBone Count Report";
// 	filterString += "&type="+type+"&flid="+celValue.substr(0,12);//+"&Keyid="+celValue;
...
// ✅ send drillCaption
filterString += '&drillCaption=' + encodeURIComponent(drillCaption);

var tableCaption = "DrillDown FishBone Count Report";
filterString += "&flid=" + celValue.substr(0,12);

//alert("filterString = " + filterString);
	processGridnew("FishBoneCount_input.fishbone",filterString,"impVscomp","pager",tableCaption,"doubleClickGrid","","impVscomp_loadComplete");	

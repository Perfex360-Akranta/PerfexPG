...
function frmKaizenPerPerson_afterLoadCallBack(){
	...
	function fnShowKaizenGraph(graphType) {
		...
		FirstLevel = 'Y';
		//if (graphType=="LINE")
			var url = "chartKaizenPerPerson.kaz?q&graphType="+graphType+"&flid="+flid+"&FirstLevel="+FirstLevel+"&type="+type;
		//else
...
function viewGrid(url,filterString)
{
	...
	{	
		...
		processGridnew("kznPerPersonPerMonth_input.kaz",filterString,"kznPerPersonGrid","pager",tableCaption,"doubleClickGrid","","kznPerPersonGrid_loadComplete");		
		return true;
	}
	return false;	
}
...
function doubleClickGrid(id){ 
	...
	var keyfieldData=jQuery("#hdnFnlnKeyid").val();
	var rowId=jQuery('#kznPerPersonGrid').jqGrid("getGridParam", 'selrow');	
	var celValue = jQuery('#kznPerPersonGrid').jqGrid ('getCell', rowId, 'KEYFIELD2'); //Company3

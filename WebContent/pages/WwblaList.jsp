...
jQuery(document).ready(function(){	//alert(1);
	...
	//fillComboBox("frmFindNode","cmbWwblPreparedby","employee.commonFilter");
	fillComboBox("frmFindNode","cmbWbblApprovedby","employee.commonFilter");
	fillComboBox("frmFindNode","cmbFismDefect","defphen.commonFilter");
	formatDateBox('dteWwblPrepareddate','dd-MMM-yyyy');
	formatDateBox('dteWwblApproveddate','dd-MMM-yyyy');
	...
	loadFunctionalLocation("WwblafunLocation", "functionalLoc.wwbla", "WwblafunLocation", "frmFindNode",dataStr);
	//alert(jQuery('#hdnRefDoctype').val());
	
	buildIndicatorTree();
	var masterId=jQuery("#txtWwblKeyid").val();//WWBL_KEYID
    //alert(jQuery('#txtWwblKeyid').val());
	
	if(masterId.trim().length>0){
		var prepdate=getFieldValue("dteWwblPrepareddate");
		var ApprDate=getFieldValue("dteWwblApproveddate");

		var PreparedDate  = prepdate.substring(0, 12);
		var ApprovedDate  = ApprDate.substring(0, 12);
	    
		setFieldValue("dteWwblApproveddate",ApprovedDate,"frmFindNode");
		setFieldValue("dteWwblPrepareddate",PreparedDate,"frmFindNode");
		}
		else{
		fillWithCurrentDate("dteWwblPrepareddate");
		fillWithCurrentDate("dteWwblApproveddate");
		}

		if(masterId.trim().length<=0){
			
	         var userlogin=jQuery('#hdnpreprdby').val(); 
	         setFieldValue("cmbWwblPreparedby",userlogin);
		     setFieldValue("cmbWbblApprovedby",userlogin);
	 ...
	 jQuery("#btnsaveFisBone").click(function()
			 {
				  ...
				  var sbu = jQuery("#frmFindNode input[id='sbu']").val(); 
				    ...
				    {	var wmrefDoc=jQuery("#hdnRefDoctype").val();
				        ...
				        saveForm('frmFindNode',"FishBoneTree_save.fishbone?&frstIdn=frstIdn&refDoc="+wmrefDoc);										
				    }	
			 });
...
function frmFindNode_successsCallback(result)
{
	...
	//alert(" After :: "+jQuery('#txtWwblKeyid').val());
...
function refTree(node)
{
 ...
 function frmFindNode_FuntLocHierarchy_SuccessCallBack(result){
	  ...
	  //alert(cellId);
	  reloadCombo("frmFindNode","cmbWwblPreparedby","employee.commonFilter?&cellId="+cellId+"&flid="+flid);
	 // reloadCombo("frmFindNode","cmbWwblPreparedby","employee.commonFilter");
	/* if(result.flId != undefined && result.flId.trim() != "" && result.flId.trim() !=null)
		{ 
		    reloadCombo("frmFindNode","cmbWwblPreparedby","employee.commonFilter?&flid="+result.cellId);
 ...
 jQuery("#btnFisBoneActionplan").click(function(){
	 //alert(1);
	 var FishMstkeyid=jQuery("#txtWwblKeyid").val();
	if(FishMstkeyid.trim().length>0)
    { 
		   var keyid=jQuery("#txtWwblKeyid").val();
		   var flid=jQuery("#flid").val();
		   var mainTask=getFieldValue("txtWwblProblem");
	...
	function frmWwblacmbwwblMachineid_onSelect(record)
	{
		//alert(123);
		//alert(record.id);
		loadFunctionalLocation("WwblafunLocation","functionalLoc.wwbla","WwblafunLocation","frmFindNode","&machId="+record.id);
	}		
...
<div>
     <div id="frmFindNodeFuntKeyIds">
		...
		<input type="hidden" id= "WwblSbuid"  name= "hdnwwblSbuid" value=""/>     
...
<td rowspan="2" style="padding-left:30px;">
		...
		<div>
			<textarea onkeyup="restrict(this.value,this.id)" onpaste="restrictPaste(this.value,this.id)"  id="txtWwblProblem" name="txtWwblProblem" style="resize:none;width:265px;text-transform:uppercase;" maxlength="600">${requestScope.newBdmTlWwblamst.wwblProblem}</textarea>
		</div>
	</td>
<td style="padding-left:30px;">
<div><label>Prepared Date</label></div>
<div><input id="dteWwblPrepareddate" name="dteWwblPrepareddate" class="easyui-datebox"  value="${requestScope.newGenTlFishbonemst.fismPrepareddate}"> </div>
</td>

<td style="padding-left:30px;">
<div><label>Prepared By</label></div>
<div><input id="cmbWwblPreparedby" name="cmbWwblPreparedby" class="easyui-combobox"   style="width:210px;"  value="${requestScope.newGenTlFishbonemst.fismPreparedby}" /> </div>  
</td>
</tr>
<tr>
...
<div><input id="dteWwblApproveddate" name="dteWwblApproveddate" class="easyui-datebox" value="${requestScope.newGenTlFishbonemst.fismApproveddate}" > </div>
</td>
<td style="padding-left:30px;display: none;">
<div><label>Approved By</label></div>
<div><input id="cmbWbblApprovedby" name="cmbWbblApprovedby" class="easyui-combobox"   style="width:210px;" value="${requestScope.newGenTlFishbonemst.fismApprovedby}" /> </div>
</td>
<td valign="bottom">
...
<td>
	<div style="margin-left:5.6%;margin-top:1%;width:100%;height:50%;font-size:12px;text-transform:uppercase;">
		...
		<div id="wwblaTree" style="margin-left:24px;float:left;width:506px;background-color:white;height:306px;height:385px\9;overflow:auto;box-shadow:3px 3px 4px rgba(0, 0, 0, 0.2);" class="sub-cntborder">
...
<div id="prevwwblaAddLayer"></div>
<div id="wwblaAddLayer" style="box-shadow:3px 3px 4px rgba(0, 0, 0, 0.2);margin-left:11%;width:550px;height:306px;margin-top: 6px;" class="sub-cntborder"></div>

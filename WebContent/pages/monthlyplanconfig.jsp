...
/*****End *****/
	...
	fillComboBox("frmFindFLNode","findFLNode","companyCombo.commonFilter" );
	/*jQuery( "#filter" ).click(function() {
		jQuery( "#dialog_wnd" ).show();
		jQuery( "#dialog_wnd" ).dialog({
			autoOpen: false,
			modal: true,
			height: 500,
			width: 750			
	...
	if( filter != null && filter.length > 0 )
	{
		...
		/*var dataString = "rowId="+machineId;
		var month = jQuery('#hdnMonth').val();
		var dataString = '?q=1&machineId='+machineId;		
		dataString += '&cmbMchid='+machineId;
		dataString += '&cmbCellid='+cellId;
		dataString += '&cmbSectid='+sectId;
		dataString += '&cmbFactid='+factId;
		dataString += '&dtFromMonth='+fromMonth;
		dataString += '&dtToMonth='+toMonth;
		dataString += '&cboRelatedTo='+relatedTo;*/
		
		processGridnew(url,filter,"MPgrid","MPpager","","monPlanDBLClick","","monthlyPlanGridComplete");
	}	
	else	{
	...
	jQuery("#findFL").combobox({
		onSelect:function(recordid){
			...
			reloadCombo("frmFindNode","findFLNode",getComboUrl(recordid.text));
		}
	});
	...
	jQuery( "#mpook" ).click(function() {
		...
		if(jQuery('#chbworkordercompln').is(':checked') == true ||jQuery('#chbviewworkorder').is(':checked') == true ||jQuery('#chbmodcompworkorder').is(':checked') == true )
		{
			...
			navigateToNextForm('generatewo_input.mpc?filterString='+filterString,'Monthly Calendar-Overall',null,{"filterString":filtStr});
			//navigateToNextForm('jhcalendar_input.jhcal'+'?filterString='+escape(filterString),"",null,perstData);
		}
			...
			var persistdata =  jQuery.parseJSON(jsonstr);
			navigateToNextForm('updatecancelgrid_input.mpc?&filterString='+filterString,'Monthly Calendar-Overall',null,{"filterString":filtStr});
		}
			...
			navigateToNextForm('updtCancelGridsel_input.mpc?&filterString='+filterString,'',null,{"filterString":filtStr});
			...
			filterStr += '&acttype='+jQuery("#hdnAEVal").val();
			
			if(jQuery('#flsFrmMonth').val() != '' && jQuery('#flsFrmMonth').val() != ' ' && jQuery('#flsFrmMonth').val() != null)
				 jsonstr = '{"cmbMchid":"'+jQuery('#hdnMachId').val()+'" ,"cmbAssmbid":"'+jQuery('#flsAssembly').val()+'","cmbFactid":"'+jQuery('#hdnFctId').val()+'" ,"cmbSectid":"'+jQuery('#hdnSectId').val() + '" ,"cmbCellid":"'+jQuery('#hdnCellId').val()+'","dtFromMonth":"'+jQuery('#flsFrmMonth').val()+'","dtToMonth":"'+jQuery('#flsToMonth').val()+'","chkMonthwise":"1"}';
			var persistdata =  jQuery.parseJSON(jsonstr);
				LoadPopUp("loadReschedule", "reschedule_input.mpc?"+filterStr, true,"54%","48%","0%","10%", "reschedule_successCallBack","Reschedule Details");
		//	navigateToNextForm('reschedule_input.mpc?&filterString='+filterString,'');
		}
...
jQuery('#sdm_click').click(function(){
	LoadPopUp("loadSDM", "sdmOpen_input.mpc?q=2" , true,"50%","37%","0%","10%", "sdm_successCallBack","Shut Down Maintainence");
});
});
function loadSDM_onClose(){
	var filtStr=jQuery.cookie("filterString");
	processGridnew("monplanconf_input.mpc","?q=2&filterString="+filtStr,"MPgrid","MPpager","","monPlanDBLClick","","monthlyPlanGridComplete");
	return true;
}
...
function viewGrid(url,filterString)
{
	...
	{			var gridurl = jQuery('#hiddenUrl').val();		
		processGridnew(gridurl,filterString,"MPgrid","MPpager","","monPlanDBLClick","","monthlyPlanGridComplete");
...
function monPlanDBLClick(id)
{
	...
	if(chkFutDate){ 
		if( jQuery("#hdnAECol").val() != null &&  jQuery("#hdnAECol").val() != '' &&  jQuery("#hdnAECol").val() != ' ')
		{ 
			openmpdialog();
		    ...
		    else if(jQuery("#hdnAEVal").val() == 'A' || jQuery("#hdnAEVal").val() =='   A')
			{
		    	...
		    	enableFields('chbgenworkorder');
		    	//enableFields('chbrescheduleact');k
...
function validateFilterSelection(filterString){
	...
	return true;
		...
		jQuery('#hiddenUrl').val(' monplanconf_input.mpc');

...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		//madhan
		 ...
		 isMSIE = /msie|trident/.test(ua),
		 isSafari = /webkit/.test(ua) || (/safari/.test(ua) && !/chrome/.test(ua));
		ts = this;
		...
		if (isMSIE && $.browser.version <= 6) {alert('');
		...
		sortData = function (index, idxcol,reload,sor){
			...
			if(ts.p.datatype == "local") {
				...
				ts.p.sortname = idxcol; // by Prasanth
				ts.p.selrow = null;
		...
		if(isMSIE) { 
			...
			/****Added by Manikandan to set default grid height in IE8*****/

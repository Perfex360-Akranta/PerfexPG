...
$.jgrid.extend({
	editCell : function (iRow,iCol, ed){
		return this.each(function (){
			...
			if (!$t.p.knv) { 
				$($t).jqGrid("GridNav");
				/***Added By Manikandan for grid click issue for IE**/
				if ($.browser.msie){
	...
	GridNav : function() {
		return this.each(function () {
			...
			$t.p.knv = $t.p.id + "_kn";
			/***position Absolute Added By Manikandan for grid click issue for IE**/
			var selection = $("<span style='width:0px;height:0px;background-color:black;position:absolute;' tabindex='0'><span tabindex='-1' style='width:0px;height:0px;background-color:grey' id='"+$t.p.knv+"'></span></span>"),
			i, kdir;
			...
			.keydown(function (e){
				...
				else if($t.p.direction == "ltr" && kdir==9){ //added by Prasanth
					if(e.shiftKey)
						kdir = 37;
					else	
						kdir = 39;
				...
				/** Added by prasanth */
				var colIndex = $t.p.iCol;
				var rowIndex = $t.p.iRow; 
				var kepressEvent = $t.id +"_keypress";
				var args = [ kdir ,rowIndex, colIndex];
				 
				try{ 
					var functionName = eval( kepressEvent );

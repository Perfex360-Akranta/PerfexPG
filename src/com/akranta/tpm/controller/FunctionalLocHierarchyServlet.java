...
public class FunctionalLocHierarchyServlet extends HttpServlet { 
	
	
	FunctionalLocnServices functionalLocnServices;
	public FunctionalLocHierarchyServlet(){
	...
	private void process(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if( action.equals("loadFuntLoc.selectFuntLoc") )
		{
			...
			try{
				FunctLocHierarchyIdentBean functLocHierarchyIdentBean = (FunctLocHierarchyIdentBean)request.getAttribute("functLocHierarchyIdentBean"); 
				...
				//fact// String factId = request.getParameter("factId");
			...
			//	CommonMessage.debugMsg("test data....");
				String teamId = request.getParameter("teamId");
				String machId = request.getParameter("machId");
				String flid = request.getParameter("flid");
			//	CommonMessage.debugMsg("loadFuntLoc.selectFuntLocteamID  "+teamId);
				...
				if (!UIUtils.isValidKeyId(flid) && !UIUtils.isValidKeyId(machId) &&
						...
						!UIUtils.isValidKeyId(locnId) && !UIUtils.isValidKeyId(compId)
						)
					flid =sessionFlid;

				CommonMessage.debugMsg("flid Flidddd333=="+flid);
				String roleId = request.getParameter("roleId");
				
				//fact// FactoryLayout factoryLayout= getAllRelatedFunctLocHierarchy(compId,locnId,factId,sbuId,pbuId,sectId,cellId,teamId,machId,flid);
				...
				//lockElements(functLocHierarchyIdentBean ,factoryLayout,request,formId,sessionFlid);
				...
				String formStr = buildFunctionlLocViewPage(divId,formId,url,factoryLayout, mode,functLocHierarchyIdentBean,isLock,enable);
			...
			CommonMessage.debugMsg("inside  getFuntLoc.selectFuntLoc ");
			HttpSession httpSession = request.getSession(false);
			...
			if(httpSession != null )
			{
				...
				CommonMessage.debugMsg(" roleIdroleId " + roleId  );
				...
				*/
				...
				//fact// FactoryLayout factoryLayout= getAllRelatedFunctLocHierarchy(compId,locnId,factId,sbuId,pbuId,sectId, cellId,teamId,mchId,flid);
				...
				if(  user.getUsrm_isadministartor().equals("Y") && userFactoryLayout != null && factoryLayout == null )
				{
					...
					//CommonMessage.debugMsg("  ----- ++ " + userFactoryLayout.getCompany().getKeyid());
					   if( factoryLayout == null)
						   factoryLayout = userFactoryLayout;
				...
				if( factoryLayout != null ){
					...
					//CommonMessage.debugMsg(functLocHierarchyIdentBean.getCompany());
				...
				lockElements(functLocHierarchyIdentBean, factoryLayout,request);
				//lockElements(functLocHierarchyIdentBean, factoryLayout,request,formId,flid);
				...
				//CommonMessage.debugMsg("dispUrl======"+dispUrl);
			}
		}
			...
			if(httpSession != null )
			{
				FunctLocHierarchyIdentBean functLocHierarchyIdentBean = (FunctLocHierarchyIdentBean)httpSession.getAttribute("functLocHierarchyIdentBean"); 
				...
				//fact// validStr.append(checkMandatoryField(functLocHierarchyIdentBean.getFactory()));
				...
				if( validStr.length()>0 ){
					...
					//fact// if( UIUtils.isValidKeyId(functLocHierarchyIdentBean.getFactory().getName())  )
					//fact// controlIds.put(functLocHierarchyIdentBean.getFactory().getName());
					...
					//CommonMessage.debugMsg("controlIds  "+controlIds.toString());
	...
	private void lockElements(FunctLocHierarchyIdentBean fLHierBean,FactoryLayout fL,HttpServletRequest request){
		...
		CommonMessage.debugMsg("formId>>>"+formId);
		CommonMessage.debugMsg("divId>>>"+divId);
		request.setAttribute("disable", isLock);
		CommonMessage.debugMsg(" isLock " + isLock);
		HttpSession httpSession = request.getSession(false);
		...
		CommonMessage.debugMsg(loginElID +"   loginFlid>>> loginFlid>>>   "+loginFlid);
		Properties prop=new Properties();
	     InputStream input=null;
		if(formId!=null)
		{
		if(formId.equals("frmFilter"))
		{
	    
		//InputStream input=new FileInputStream("com/akranta/tpm/resources/ApplicationConfig.properties");/perfexitc_dev/src/com/akranta/tpm/resources/Functionallocnval.properties
  input=this.getClass().getClassLoader().getResourceAsStream("com/akranta/tpm/resources/Functionallocnval.properties");
		try {
			prop.load(input);
		} catch (IOException e) {
			// TODO Auto-generated catch block
   ...
   // }
        ...
        if(prop!=null)
        {
//		 fnval=prop.containsKey(loginFlid); 
		 fnval = loginFlid != null && prop.containsKey(loginFlid);//madhan
		 CommonMessage.debugMsg(" 08 fnval>> fnval>>fnval>>fnval>>"+fnval);
		 if(fnval==true)
		 {
			 fnln=prop.getProperty(loginFlid);
		 }
	CommonMessage.debugMsg(" 09 fnln>>>> fnval>>fnval>>fnval>>fnval>>"+fnln);
        }
		
		
		int n = loginElID != null ?  loginElID.split("-").length : 0;
		String enable = request.getParameter("enable");
		...
		CommonMessage.debugMsg(" Checking "+enable);
		
	
		if(  "LCN".equals(enable))  return ;
		
		if(  fL.getLocation() != null && UIUtils.isValidKeyId( fL.getLocation().getKeyid() ) && n >1){
			fLHierBean.getLocation().setDisable(true);
		}
		
		if(  "SBU".equals(enable))  return ;
		
		if(  fL.getSbu() != null &&  UIUtils.isValidKeyId( fL.getSbu().getKeyid() )&& n >2){
			fLHierBean.getSbu().setDisable(true);
		}
		
		if(  "PBU".equals(enable))  return ;
		
		if(  fL.getPbu() != null &&  UIUtils.isValidKeyId( fL.getPbu().getKeyid())&& n >3){
			fLHierBean.getPbu().setDisable(true);
		}
		if(  "DMT".equals(enable))  return ;
		
		if(  fL.getSection() != null &&  UIUtils.isValidKeyId(  fL.getSection().getKeyid())&& n >4){
			...
			if(formId.equals("frmFilter"))
			{
				if(fnval==true)
				{
					if(fnln.equals("DMT"))
					{
						fLHierBean.getSection().setDisable(false);
		...
		if(  "JH".equals(enable))  return ;
		if(  fL.getCell() != null &&  UIUtils.isValidKeyId(  fL.getCell().getKeyid() )&& n >5){
			...
			if(formId.equals("frmFilter"))
			{
				if(fnval==true)
				{
					if(fnln.equals("JH"))
					{
						fLHierBean.getCell().setDisable(false);
					}
					else{
						if(fnln.equals("DMT"))
						{
		...
		if(  "MCH".equals(enable))  return ;
		if(  fL.getMachine() != null &&  UIUtils.isValidKeyId(  fL.getMachine().getKeyid() )&& n >6){
			fLHierBean.getMachine().setDisable(true);
		}
	}
	
private void lockElements1(FunctLocHierarchyIdentBean fLHierBean,FactoryLayout fL,HttpServletRequest request) throws IOException{
	...
	//	//CommonMessage.debugMsg("loginFlid>>>"+loginFlid);
		...
		////CommonMessage.debugMsg("fnval>>>>"+fnval);
        }
		
		////CommonMessage.debugMsg(prop.getProperty(loginFlid));
		//String FNLNVal = UIUtils.getPropertyValue("com.akranta.tpm.resources.ApplicationConfig", loginFlid);
	...
	private JSONObject getFuntHierarchyKeyids(FactoryLayout factoryLayout){
		...
		if( factoryLayout != null ){
			...
			//fact// functHierarchy.put("factId", factoryLayout.getFactory().getKeyid());
...
//fact// 	private FactoryLayout getAllRelatedFunctLocHierarchy(String compId,String locnId, String factId,String sbuId,String pbuId,String sectId,
private FactoryLayout getAllRelatedFunctLocHierarchy(String compId,String locnId, String sbuId,String pbuId,String sectId,
		...
		//fact// if( UIUtils.isValidKeyId(compId) || UIUtils.isValidKeyId(locnId) || UIUtils.isValidKeyId(factId) ||  UIUtils.isValidKeyId(sbuId) ||		
		if( UIUtils.isValidKeyId(compId) || UIUtils.isValidKeyId(locnId) ||  UIUtils.isValidKeyId(sbuId) ||
				UIUtils.isValidKeyId(pbuId) ||UIUtils.isValidKeyId(sectId) || UIUtils.isValidKeyId(cellId) || UIUtils.isValidKeyId(machId)  || UIUtils.isValidKeyId(flid))
		{
			...
			//fact// factoryLayout.setFactory(getExistFuntLocnElement(factId));
			...
			//CommonMessage.debugMsg("flid...."+factoryLayout.getFlid());
		}
	...
	private void populateValues(FunctLocHierarchyIdentBean functLocHierarchyIdentBean, HttpServletRequest request){
		...
		if( functLocHierarchyIdentBean.getLocation() != null && functLocHierarchyIdentBean.getLocation().getName() != null){
			...
			tmpFld = request.getParameter(paramName);
			CommonMessage.debugMsg("  locationId " + tmpFld);
...
*/		
		...
		/*if( functLocHierarchyIdentBean.getFlid() != null && functLocHierarchyIdentBean.getFlid().getName() != null){
			...
			CommonMessage.debugMsg("FunctionLocation Param NAME  :" +paramName);
			if( ! UIUtils.isValidKeyId(paramName) )
				paramName = "hdnFunctLocFL";
			CommonMessage.debugMsg("FunctionLocation Param NAMEs  :" +paramName);
			tmpFld = request.getParameter(paramName);
		...
		*/
		if( functLocHierarchyIdentBean.getMachine() != null && functLocHierarchyIdentBean.getMachine().getName() != null){
			...
			CommonMessage.debugMsg("FunctionLocation tmpFld NAME  :" +tmpFld);
			tmpFld = request.getParameter(paramName);
			functLocHierarchyIdentBean.getMachine().setValue(tmpFld);
		}
	}
	
	private String buildFunctionlLocViewPage(String divId, String formId, String url,FactoryLayout factoryLayout, FormModes mode,FunctLocHierarchyIdentBean functLocHierarchyIdentBean,String isLock,String enable){
...
//			form.append("jQuery('#btn"+ formId + "mainFunLoc').live('click',function(){");
			form.append("jQuery(document).on('click','#btn"+ formId + "mainFunLoc', function() {");
...
//				form.append("jQuery('#lin"+formId +"Company').live('click',function(){");
				form.append("jQuery(document).on('click','#lin"+ formId + "Company', function() {");
...
//				form.append("jQuery('#lin"+formId +"Location').live('click',function(){");
				form.append("jQuery(document).on('click','#lin"+ formId + "Location', function() {");
...
//				form.append("jQuery('#lin"+formId +"Sbu').live('click',function(){");
				form.append("jQuery(document).on('click','#lin"+ formId + "Sbu', function() {");
				form.append(formId+"_fillFunloc(7);");
				form.append("}); ");
			}
...
//				form.append("jQuery('#lin"+formId +"Pbu').live('click',function(){");
				form.append("jQuery(document).on('click','#lin"+ formId + "Pbu', function() {");
...
//				form.append("jQuery('#lin"+formId +"Section').live('click',function(){");
				form.append("jQuery(document).on('click','#lin"+ formId + "Section', function() {");
...
//				form.append("jQuery('#lin"+formId +"Cell').live('click',function(){");
				form.append("jQuery(document).on('click','#lin"+ formId + "Cell', function() {");
				form.append(formId+"_fillFunloc(63);");
				form.append("}); ");
			}	
...
//				form.append("jQuery('#lin"+formId +"Machine').live('click',function(){");
				form.append("jQuery(document).on('click','#lin"+ formId + "Machine', function() {");
		...
		//fact//form.append("if( (selCntrl & 4 ) != 0 ) factId = jQuery('#"+ formId + "  input[id=\"factory\"]').val();");
		...
		//fact// form.append(" if( factId != null && factId.trim() != '' &&  factId.length >0 ) ");
		//fact// form.append("      dataStr +='&factId='+factId ;");
		...
		if( enable != null)
			form.append("      dataStr +='&enable=" + enable  + "' ;");
	...
	//	form.append("<input id='"+ formId + "linShowFuntLocHier' type='button' /> ");
		...
		//CommonMessage.debugMsg("ptrint stmt....."+form.toString());
	...
	private String getFunctLocHiddenFieldIds(String divId,String formId ,FactoryLayout factoryLayout,FunctLocHierarchyIdentBean functLocHierarchyIdentBean ){
		StringBuffer str = new StringBuffer();
		...
		if( factoryLayout != null){
			...
			//fact// if( factoryLayout.getFactory() != null)
			//fact// str.append(getDefaultElementHtml(formId,"factory",factoryLayout.getFactory().getKeyid()));
			...
			if(factoryLayout.getFlid() != null){
				CommonMessage.debugMsg("keyid...."+factoryLayout.getFlid().getKeyid());
			...
			//fact// if( factoryLayout.getFactory() != null && UIUtils.isValidKeyId(factoryLayout.getFactory().getKeyid() ))
			//fact// 	str.append(" args.factId =\""+ factoryLayout.getFactory().getKeyid() +"\";");
			...
			//CommonMessage.debugMsg(" str try == " + str);
		}
		return str.toString();
	}
	private String getFunctLocDisplayString(FactoryLayout factoryLayout,String formId){
		...
		if( factoryLayout != null ){
			...
			//fact// form.append(getDisplayString(formId,factoryLayout.getFactory(),"Factory" ));

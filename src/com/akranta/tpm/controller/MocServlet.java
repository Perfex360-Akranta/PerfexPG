...
public class MocServlet extends HttpServlet {
   ...
   private void process(HttpServletRequest request,HttpServletResponse response) throws Exception{
	   ...
	   // mocServiceImpl=(mocServiceImpl)UIUtils.getServiceObject(request,"mocServiceImpl");
	    ...
	    mocService=(MocServiceImpl)UIUtils.getServiceObject(request, "MocServiceImpl");
	    ...
	    actionPlanRptService=(ActionPlanRptServiceImpl) UIUtils.getServiceObject(request, "ActionPlanRptServiceImpl");
	    	...
	    	if(action.equals("Questionaire_input.nmoc")){ 
		CommonMessage.debugMsg("Action::::"+action);
		  UIUtils.forwardRequest(request, response,"pages/Questionaire.jsp"); 
		
	}
	else if(action.equals("Questionaire_getCol.nmoc")){
		{	
	/*	CommonFilter commonFilter = populateCommonFilter(request,"QuestionaireCommonFilter",true);
	...
	//	JSONObject jsonObject = getTableModel(visualGridList);
		JqGridTableModel jqGridTableModel = new JqGridTableModel();
  		...
  		String[] colHeader = visualGridList.get(2);
  		String[] colHeaderCond = visualGridList.get(1);
		...
		httpSession.setAttribute("VisualColmodel", colModel);
		out.println(colModel);	*/
			try{
				PrintWriter out = response.getWriter();
			
				out.print(UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC","Questionaire"));
				CommonMessage.debugMsg(UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC","Questionaire")); 
...
else if(action.equals("Questionaire_getData.nmoc")){
/*	try
	{
	
	UIUtils.displayRequestParamsValue(request);	
	
	CommonFilter commonFilter = populateCommonFilter(request,"Questionaire",false);
	
	List<String[]> visualGridList = mocService.getQuestionaire(commonFilter);
	PrintWriter out = response.getWriter();
		JSONObject visualQueryData = UIUtils.convertToJqGridTableObject(visualGridList,request,3,0); 
...
else if(action.equals("Questionaire_save.nmoc")){
	 String MocKeyid=request.getParameter("MocKeyid");
	    CommonMessage.debugMsg("MOC Keyid::::"+MocKeyid);
	saveQuestionsdata(request,response,MocKeyid);
}

else if(action.equals("PSSRRecommentationsCount.nmoc")){
	PrintWriter out=response.getWriter();
	String Mockeyid=request.getParameter("MocKeyid");
...
else if(action.equals("PSITeamMailTrigger.nmoc")){
	PrintWriter out=response.getWriter();
	String Mockeyid=request.getParameter("MocKeyid");
...
else if(action.equals("WhatIfHazopRecommentationsCount.nmoc")){
	PrintWriter out=response.getWriter();
	String Mockeyid=request.getParameter("MocKeyid");
...
else if(action.equals("InitialApprovalCount.nmoc")){
	PrintWriter out=response.getWriter();
	String Mockeyid=request.getParameter("MocKeyid");
...
else if(action.equals("HazopApprovalCount.nmoc")){
	PrintWriter out=response.getWriter();
	String Mockeyid=request.getParameter("MocKeyid");
...
else if(action.equals("FinalApprovalCount.nmoc")){
	PrintWriter out=response.getWriter();
	String Mockeyid=request.getParameter("MocKeyid");
...
else if(action.equals("MOCClosureCheck.nmoc")){
	PrintWriter out=response.getWriter();
	String Mockeyid=request.getParameter("MocKeyid");
...
else if(action.equals("MocCreationGrid_input.nmoc")){
	...
	RequestDispatcher rd = request.getRequestDispatcher("/pages/MOCGrid.jsp");
	rd.forward(request, response);
}
else if(action.equals("MocCreationGrid_getCol.nmoc")){
...
else if(action.equals("MocCreationGrid_getData.nmoc")){
	
	PrintWriter out = response.getWriter();	
	CommonFilter commonFilter=populateCommonFilter(request, "MocCommonFilter", false);
	CommonMessage.debugMsg("in  Data"+action);
	List<String[]> saralViewData = mocService.getMocRelatedData(commonFilter);
	    JSONObject dataJson = UIUtils.convertToJqGridTableObject(saralViewData,request,3,0,commonFilter.getTotalRecordCnt()+3);
	out.print(dataJson);
	commonFilter.setViewClick('Y');
	httpSession.removeAttribute("MocCommonFilter");
	httpSession.setAttribute("MocCommonFilter", commonFilter);
}	    	
else if(action.equals("MocCreationGrid_getExcel.nmoc")){
...
else if( action.equals("WhatIFEntry_getCol.nmoc") )
{

	try{
		PrintWriter out = response.getWriter();
		CommonFunctions.debugMsg("RiskAssesment_getCol.risk.............");
		out.print(UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC","WhatIf"));
		CommonMessage.debugMsg(UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC","WhatIf")); 
...
else if( action.equals("getRiskLevel.nmoc"))
{				
	try{
...
else if( action.equals("getRiskLevel1.nmoc"))
{				
	try{
		String riskVal =request.getParameter("riskval");
		CommonMessage.debugMsg("The Risk val:::"+riskVal);
		...
		e.printStackTrace();
	    	...
	    	else if( action.equals("getRiskLevel2.nmoc"))
	    	{				
	    		try{
	    			String riskVal =request.getParameter("riskval");
	    			CommonMessage.debugMsg("The Risk val:::"+riskVal);
	    	...
	    	else if( action.equals("getRiskLevel3.nmoc"))
	    	{				
	    		try{
	    			String riskVal =request.getParameter("riskval");
	    			CommonMessage.debugMsg("The Risk val:::"+riskVal);
	    	...
	    	else if( action.equals("HazopEntry_getData.nmoc") )
	    		{
	    			try {
	    				CommonFilter commonFilter = new CommonFilter();
	    		...
	    		else if( action.equals("WhatIFEntry_getData.nmoc") )
	    		{
	    			try {
	    				CommonFilter commonFilter = new CommonFilter();
	    				String keyid =request.getParameter("MocKeyid");
...
else if( action.equals("HazopEntry_getCol.nmoc") )
{

	try{
		PrintWriter out = response.getWriter();
		CommonFunctions.debugMsg("RiskAssesment_getCol.risk.............");
		out.print(UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC","Hazop"));
		CommonMessage.debugMsg(UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC","Hazop")); 
...
{				
  ...
  if(action.equals("ChangeRequest_input.nmoc")){
		   ...
		   /*String EHSHead=mocService.getEHSHead(Sugflid);
		   String EHSManager=mocService.getEHSManager(Sugflid);*/
	...
	CommonMessage.debugMsg("mocRfcmst"+mocRfcmst+"whatif"+whatifMst);
	    ...
	    CommonMessage.debugMsg("Befor MOC JSP");
	  	    	UIUtils.forwardRequest(request, response,"/pages/MOC.jsp");
	  	    }
  else if (action.equals("ChangeRequest_save.nmoc")){
	  SaveMOC(request,response);
  }
  
  else if(action.equals("ChangeRequestBasis_getCol.nmoc")){
  	String unsafeact=request.getParameter("unsafeact");
		...
		PrintWriter out = response.getWriter();
		String tableModel = UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC","QCLogging");
			JSONObject colmodel = JSONObject.fromString(tableModel);
			out.println(colmodel);	
	 }
	   
	 else if(action.equals("ChangeRequestBasis_getData.nmoc")){
			
		/*	try {
				PrintWriter out = response.getWriter();
		CommonFilter commonFilter = populateCommonFilter(request,"ChangeRequest CommonFilter",false);
	    List<String[]> safety = newCRService.getCRBasis(commonFilter);
				JSONObject Safety = UIUtils.convertToJqGridTableObject(safety, request, 3, 0);
					//httpSession.removeAttribute("genTlSusamst");
					out.println(Safety);
			} catch (Exception e) {
				e.printStackTrace();
			}*/
	 ...
	 else if(action.equals("InitialApproval_getCol.nmoc")){
			
			String colModel = UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC", "InitialApproval");
			response.getWriter().print(colModel);
			response.getWriter().close();
		}
	 else if(action.equals("InitialApproval_getData.nmoc")){
			String MocKeyid=request.getParameter("MocKeyid");
	 ...
	 else if(action.equals("HazopApproval_getCol.nmoc")){
			
			String colModel = UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC", "HazopApproval");
			response.getWriter().print(colModel);
			response.getWriter().close();
		}
	 else if(action.equals("HazopApproval_getData.nmoc")){
			String MocKeyid=request.getParameter("MocKeyid");
			CommonMessage.debugMsg("MocKeyid in Approval"+MocKeyid); 
	 ...
	 else if(action.equals("FinalApproval_getCol.nmoc")){
			
			String colModel = UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC", "FinalApproval");
			response.getWriter().print(colModel);
			response.getWriter().close();
		}
	 else if(action.equals("FinalApproval_getData.nmoc")){
			String MocKeyid=request.getParameter("MocKeyid");
			CommonMessage.debugMsg("MocKeyid in Approval"+MocKeyid); 
	 ...
	 else if(action.equals("GuideWord.nmoc")){
		   try{
	 ...
	 else if(action.equals("MOCTeamSusscess_getCol.nmoc")){
		 ...
		 CommonMessage.debugMsg("Success Call Back");
	      ...
	      String MocKeyId=request.getParameter("MocKeyid");
			...
			String tableModel = UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC","MOCTeam");
			JSONObject colmodel = JSONObject.fromString(tableModel); 
			response.setContentType("text/html");
			   out.print(colmodel);
			   out.close();   
	 }
	 else if (action.equals("MOCTeamSusscess_getData.nmoc")) {
...
/*  else if (action.equals("PssrReccommendation_save.nmoc")){
	  SavePssrReccommendation(request,response);  
  }*/
  
  else if (action.equals("MocReccommendation_save.nmoc")){
	  SaveMOCReccommendation(request,response);  
  }
  else if (action.equals("MocPSSRChkReccommendation_save.nmoc")){
	  SaveMOCPSSRCHReccommendation(request,response);  
  }
  
	 else if (action.equals("InitialApproval_save.nmoc")){
	
		 SaveInitialApproval(request,response);

	 }
  
	 else if (action.equals("HazopApproval_save.nmoc")){
			
		 SaveHazopApproval(request,response);

	 }
  
	 else if (action.equals("FinalApproval_save.nmoc")){
			
		 SaveFinalApproval(request,response);

	 }
	 else if(action.equals("Whatif_remove.nmoc")){
		 DeleteWhatifRow(request,response);
	 }
	 else if(action.equals("Hazop_remove.nmoc")){
		 DeleteHazopRow(request,response);
	 }
  
	 else if (action.equals("MOCTeam_getCol.nmoc")) { 
	 ...
	 else if (action.equals("MOCTeam_getData.nmoc")) {
			//	CommonMessage.debugMsg("get data method");
		 CommonFilter commonFilter = populateCommonFilter(request,"MOCTeam",false);
				try {
	 ...
	 else if(action.equals("managementofchangetype.nmoc")){
		  /* try{
			    ComboFilter combofilter=UIUtils.fillComboFilter(request);
		 ...
		 PrintWriter out = response.getWriter(); 
		 out .println(UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC", "MocType"));

	   }
	 else if(action.equals("questionaireYesNo.nmoc")){
		
		 CommonMessage.debugMsg("In type get Col");
		 PrintWriter out = response.getWriter(); 
		 out .println(UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC", "YesNo"));

	   }
 
  
	 else if(action.equals("managementofchangenature.nmoc")){
		 CommonMessage.debugMsg("In type get Col"+action);
		 PrintWriter out = response.getWriter(); 
		 out .println(UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC", "MocNature"));
	   }
	 
	 else if(action.equals("managementofchangetypenew.nmoc")){
		  try{
	 ...
	 else if(action.equals("managementofchangenaturenew.nmoc")){
		  try{
			  CommonMessage.debugMsg("In type combobox Col"+action);
			    ComboFilter combofilter=UIUtils.fillComboFilter(request);
			    List<ComboBox>projectList=mocService.getMocTypeComboList(combofilter);
		...
		else if (action.equals("BasisofChange_getCol.nmoc")) {
		 CommonMessage.debugMsg("achuievenen");
			...
			request.setAttribute("detailsid", detailsid);
				...
				String tableModel = UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC","BasisofChange");
				JSONObject colmodel = JSONObject.fromString(tableModel);
				
				out.println(colmodel);
		} else if (action.equals("BasisofChange_getData.nmoc")) {
		//	CommonMessage.debugMsg("get data method");
			String MocKeyid=request.getParameter("MocKeyid");
		...
		else if (action.equals("HazopRecommendations_getCol.nmoc")){
			CommonMessage.debugMsg("IN Recc");
			
				PrintWriter out = response.getWriter();
				String tableModel = UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC","HazopReccommend");
				JSONObject colmodel = JSONObject.fromString(tableModel);
				
				out.println(colmodel);	
		}
else if (action.equals("HazopRecommendations_getData.nmoc")){
		...
		else if (action.equals("PSSRRecommendations_getCol.nmoc")){
			CommonMessage.debugMsg("IN Recc");
			
				PrintWriter out = response.getWriter();
				String tableModel = UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC","PSSRREccommend");
				JSONObject colmodel = JSONObject.fromString(tableModel);
				
				out.println(colmodel);	
		}
  
else if (action.equals("PSSRRecommendations_getData.nmoc")){
		...
		else if (action.equals("MocClosure_getCol.nmoc")){
			CommonMessage.debugMsg("IN MOC CLosure");
		/*	PrintWriter out = response.getWriter();
			String tableModel = UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC","MOCClosure");
			CommonMessage.debugMsg("IN MOC CLosure");
			JSONObject colmodel = JSONObject.fromString(tableModel);*/
			PrintWriter out = response.getWriter();
			String tableModel = UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC","MOCClosure");
		...
		else if (action.equals("MocClosure_getData.nmoc")){
		...
		else if (action.equals("PssrChecklist_input.nmoc")){
			 UIUtils.forwardRequest(request, response,"pages/Pssrchecklist.jsp"); 	
		}
		else if (action.equals("PssrChecklist_getCol.nmoc")){
			PrintWriter out = response.getWriter();
			String tableModel = UIUtils.getPropertyValue("com.akranta.tpm.resources.RFC","PssrCheckList");
			JSONObject colmodel = JSONObject.fromString(tableModel);
			
			out.println(colmodel);	
		}
		else if (action.equals("PssrChecklist_getData.nmoc")){
		 ...
		 else if(action.equals("functionalLoc.nmoc"))
			{
		 ...
		 else if(action.equals("MOCExcelView_Excelview.nmoc")){
			 try{
			 String format=ExcelUtils.getFormat(request);
				 ...
				 Workbook wb = mocService.NewDashboardExcelView(format,path,MocKeyid,flid,DMT,JH,CurrentDate,NOC,Type,Detail,PCHange,Title,Initiator,MOCDate,WhatifFacility,WhatifTeam,WhatifDate,Suggestion,SuggestionId,HazopFacility,HazopTeam,HazopNode,HazopDesign,HazopDate,pidNo,PssrFacility,MOCDetails);
			     format = ".xlsx";
   ...
   private void SendMailToPSI(HttpServletRequest request, HttpServletResponse response, String mailIds,
	   ...
	   CommonMessage.debugMsg("---------Sending Mail To PSI for MOC Information----------");
		 ...
		 CommonMessage.debugMsg("MOCtitle :" +mOCtitle+"cR="+cR); 
			try {
					//CommonMessage.debugMsg("E-Mail Id  :" +UIUtils.isValidEmail(emailId) + "  " + emailId);
...
@SuppressWarnings({ "unchecked", "unused" })   
 	private void SaveMOCPSSRCHReccommendation(HttpServletRequest request, HttpServletResponse response) throws IOException {
         	...
         	try
               {
     	    	...
     	    	{	
     	    		MOCPssrReccommend existMOCPssrReccommend=(MOCPssrReccommend)httpSession.getAttribute("MOCReccommendation");
       		...
       		newMOCPssrReccommend =(MOCPssrReccommend)UIUtils.setBeanProperties((Object)newMOCPssrReccommend,request);	   	         
   	...
   	/** save JsonARR conversion For Detail **/
       		...
       		if(UIUtils.isValidKeyId(paramJsonArrNew) )
       		{
       		 ...
       		 momActionpladtlJSon = JSONArray.fromString(paramJsonArrNew);
       			 ...
       			 MOCPssrReccommendList=(List<MOCPssrReccommend>)UIUtils.convertJSONArrToList(newMOCPssrReccommend, ReccommendList);
   ...
   /**conversion END**/	
     		    ...
     		    if( ReccomendId.length()>0)
     			 {	
     		    	CommonMessage.debugMsg("In Update Reccommendation");
     		    		...
     		    		existMOCPssrReccommend=mocService.UpdatePSSRReccommendation(MocKeyidRec,Pssrdetailid,targetDate,ActionPlanStatus,Responsiblity,Reccommendation);
     			 }   	 		
     		    	
     		  	else{
     		  	CommonMessage.debugMsg("In Create Reccommendation");
     		  	MOCPssrReccommendList=mocService.createPSSRCheckReccommendation(MOCPssrReccommendList,MocKeyidRec,Pssrdetailid);
     		   String PrrrKeyid=mocService.getPssrKeyid(MocKeyidRec);
     		    	...
     		    	if(Reccommendation!=null && Reccommendation.length()>0 ){
     	  			existGenTlActionplanmst=mocService.createActionPlan(newActionplanmst, newActionplandtl,MocKeyidRec,PrrrKeyid);
   ...
   @SuppressWarnings({ "unchecked", "unused" })   
      	...
      	CommonMessage.debugMsg("In reccm save");
      	  HttpSession httpSession = request.getSession(false);
          	...
          	try
        		...
        		newActionplanmst.setAplmMaintask(Reccommendation);
        		newActionplanmst.setAplmElementid(elementid);
        		newMOCReccommendation =(MOCReccommendation)UIUtils.setBeanProperties((Object)newMOCReccommendation,request);	   	         
    	...
    	/** save JsonARR conversion For Detail **/
        		...
        		if(UIUtils.isValidKeyId(paramJsonArrNew) )
        		{
        			...
        			ReccommendList = JSONArray.fromString(paramJsonArrNew);
        			    ...
        			    momActionpladtlJSon = JSONArray.fromString(paramJsonArrNew);
        			    CommonMessage.debugMsg("Inside the JSOn");
        			  MOCReccommendationList=(List<MOCReccommendation>)UIUtils.convertJSONArrToList(newMOCReccommendation, ReccommendList);
    					MomActionplandtl=(List<GenTlActionplandtl>)UIUtils.convertJSONArrToList(newActionplandtl, momActionpladtlJSon);
    					newActionplandtl.setActionplanlist(MomActionplandtl);	
      		    ...
      		    if( ReccomendId.length()>0)
      			 {	
      		    	CommonMessage.debugMsg("In Update Reccommendation");
      		    		...
      		    		existMOCReccommendation=mocService.UpdateReccommendation(MocKeyidRec,detailid,targetDate,ActionPlanStatus,Responsiblity);
      			 }   	 		
      		    	
      		  	else{
      		  	CommonMessage.debugMsg("In Create Reccommendation");
      		    	MOCReccommendationList=mocService.createReccommendation(MOCReccommendationList,MocKeyidRec,detailid);
          	   		
      		    	if(Reccommendation!=null && Reccommendation.length()>0 ){
      	  			existGenTlActionplanmst=mocService.createActionPlan(newActionplanmst, newActionplandtl,MocKeyidRec,detailid);
	...
	private void SavePssrReccommendation(HttpServletRequest request, HttpServletResponse response) throws IOException {
    	  HttpSession httpSession = request.getSession(false);
      	ServletOutputStream out = response.getOutputStream();
      	AdmTlUsermst user = UIUtils.getLoginUser(request);    	
      	try
            {
  	    	if( httpSession != null && user != null)
  	    	{	
  	    	MOCPssrReccommend existMOCPssrReccommend=(MOCPssrReccommend)httpSession.getAttribute("MOCPssrReccommend");
    		//GenTlMomdtl existGenTlMomdtl=(GenTlMomdtl)httpSession.getAttribute("GenTlMOmdtl");
    		GenTlActionplandtl existGenTlActionplandtl = (GenTlActionplandtl) httpSession.getAttribute("newGenTlActionplandtl");
    		GenTlActionplanmst existGenTlActionplanmst = (GenTlActionplanmst) httpSession.getAttribute("newGenTlActionplanmst");
    		MOCPssrReccommend newMOCPssrReccommend = new MOCPssrReccommend();
    		...
    		newActionplanmst.setAplmCreatedby(user.getUsrm_ccno());
			   ...
			   String paramJsonArrNew=request.getParameter("pssrReccommendconvert");
    		String Rowid = request.getParameter("rowid");
    		String row=request.getParameter("row");
    		String momactnpln = request.getParameter("momactnpln");
...
*//**conversion END**//*	
  		    ...
  		    if( Reccommendation!= null)
  			 {	
  		    	MOCPssrReccommendList=mocService.createMocReccommendation(MOCPssrReccommendList,MocKeyidRec);
  		    	 String PrrrKeyid=mocService.getPssrKeyid(MocKeyidRec);
   	   		    CommonMessage.debugMsg("The"+PrrrKeyid);
				  ...
				  {
					existMOCPssrReccommend = NewmomeetingService.update(newGenTlMommst,existGenTlMommst, momeetingBean);
					...
					existGenTlActionplanmst=NewmomeetingService.updateActionPlan(newActionplanmst, newActionplandtl,newGenTlMommst,Rowid,ActionPlanId,ActionplanDetailId);
					}
					insert = false;
				  }					
    	   ...
    	   HttpSession httpSession = request.getSession(false);
		  	...
		  	{	
		  		try{
		  	 		...
		  	 		MocTeamConfigmst existMocTeamConfigmst=(MocTeamConfigmst)httpSession.getAttribute("MocTeamConfigmst");
		  			...
		  			if(keyid.length()>0){
		  				...
		  				existMocTeamConfigmst=mocService.getFinalApproval(mocTeamConfigmst,existMocTeamConfigmst,keyid,EmpId,status,date,remarks,MocKeyId);
	      ...
	      HttpSession httpSession = request.getSession(false);
			  	...
			  	{	
			  		try{
			  			...
			  			if(keyid.length()>0){
			  				CommonMessage.debugMsg("Inside the Update");
			  				existMocTeamConfigmst=mocService.getHazopApproval(mocTeamConfigmst,existMocTeamConfigmst,keyid,EmpId,status,date,remarks,MocKeyId);
			  				saveMsg="Data Updated Successfully";
			  			}
	...
	// TODO Auto-generated method stub
      HttpSession httpSession = request.getSession(false);
		  	...
		  	{	
		  		try{
		  			...
		  			if(keyid.length()>0){
		  				CommonMessage.debugMsg("Inside the Update");
		  				existMocTeamConfigmst=mocService.getInitialApproval(mocTeamConfigmst,existMocTeamConfigmst,keyid,EmpId,status,date,remarks,MocKeyId);
		  				saveMsg="Data Updated Successfully";
		  			}
	 ...
	 private void SendMailToJHLeader(HttpServletRequest request, HttpServletResponse response,String emailId,String SuggestionNo,String suggestion,String MocKeyid,String MOCtitle) throws ServletException, IOException,Exception{
	...
	private CommonFilter populateCommonFilter(HttpServletRequest request, String string, boolean b) {
  	...
  	String MocType=request.getParameter("MocType");
  	   String MocKeyidQuestionnaire=request.getParameter("MocKeyid");
  		...
  		{ 
      		MocRfcmst mocRfcmst=new MocRfcmst();
  		MocRfcmst  existMocRfcmst =(MocRfcmst)httpSession.getAttribute("mocRfcmst");
  		mocRfcmst=(MocRfcmst)UIUtils.setBeanProperties((Object)mocRfcmst,request);
  					...
  					try {
  					    if(title.equals("MOC Team")){
  					        if(gridval != null){
  					            ...
  					            List<MocTeamConfigmst> MocTeamConfigmst = null;	
  					            
  					            if(UIUtils.isValidKeyId(gridval)){
  					                teamList = JSONArray.fromString(gridval);
  					                CommonMessage.debugMsg("EmployeeList" + gridval);
  					                MocTeamConfigmst = (List<MocTeamConfigmst>)UIUtils.convertJSONArrToList(mocTeamConfigmst, teamList);	
  					                CommonMessage.debugMsg("MocTeamConfigmst ::::" + MocTeamConfigmst);
  					            }
...
//  					            }
  					        ...
  					        CommonMessage.debugMsg("Final mocRfcmst.getRfcmKeyid() :: " + mocRfcmst.getRfcmKeyid());
  					            ...
  					            if(mocRfcmst.getRfcmKeyid() == null || mocRfcmst.getRfcmKeyid().trim().isEmpty()) {
  					                ...
  					                CommonMessage.debugMsg("mocRfcmst.getRfcmKeyid() before create: " + mocRfcmst.getRfcmKeyid());

  					                existMocRfcmst = mocService.createMOC(
  					                    mocRfcmst, existMocRfcmst, mocRfcBasismst, mocTeamConfigmst
  					                );

  					                MocTeamConfigmst = mocService.createMOCTeam(
  					                    MocTeamConfigmst, createdBy, existMocRfcmst  
  					                );
  					                ...
  					                MocTeamConfigmst = mocService.createMOCTeam(
  					                    MocTeamConfigmst, createdBy, mocRfcmst
  					                );
  							...
  							MocRfcBasismst Employeelink= new MocRfcBasismst();
  		   		    		...
  		   		    		if(UIUtils.isValidKeyId(basisJsonArrnew)){
  		   		    		  ...
  		   		    		  CommonMessage.debugMsg("EmployeeList"+EmployeeList);
  		   		    		   EmployeeAddList=(List<MocRfcBasismst>)UIUtils.convertJSONArrToList(Employeelink,EmployeeList);		    		   
  		   		    		
  		   		    			EmployeeAddList=mocService.createBasis(EmployeeAddList,keyId);
  		   		    		 
  		   		    			existMocRfcmst=mocService.updateMOC(mocRfcmst,existMocRfcmst,keyId,Nature,Desc,Detail,Dmt,Jh,Initiator,MOCTitle,MocType);
							...
							CommonMessage.debugMsg("IN RFC");
							   String MocKeyidnew=request.getParameter("MocKeyid");
							   CommonMessage.debugMsg("MocKeyidnew"+MocKeyidnew);
							 String basisJsonArrnew=request.getParameter("BasisDetails");
							MocRfcBasismst Employeelink= new MocRfcBasismst();
		   		    		JSONArray EmployeeList=null; 
		   		    		List<MocRfcBasismst> EmployeeAddList = null;	
		   		    		...
		   		    		//	existMocRfcmst.setRfcmKeyid(MocKeyidnew);
		   		    			existMocRfcmst=mocService.updateMOC(mocRfcmst,existMocRfcmst,MocKeyidnew,Nature,Desc,Detail,Dmt,Jh,Initiator,MOCTitle,MocType);
							...
							JSONObject returnData = new JSONObject();//
							returnData.put("successData", successData);
							returnData.put("keyId",MocKeyidnew);
							returnData.put("formClear",false);
			    			...
			    			CommonMessage.debugMsg("MocKeyId"+keyId);
							   ...
							   JSONArray EmployeeList=null; 
			   		    		...
			   		    		MocRfQuestionsList=(List<MocRfQuestions>)UIUtils.convertJSONArrToList(mocRfQuestions,EmployeeList);	
							...
							if( ! UIUtils.isValidKeyId (newwhatifMst.getWifmKeyid() )  ){	
								...
								existWhatifMst=mocService.UpdateWhatif(newwhatifMst,existWhatifMst,CreatedBy,WhatifKeyId);
								savemsg="Data Updated Successfully";					
							}
				          ...
				          if(mocKeyIdFromRequest != null && !mocKeyIdFromRequest.trim().isEmpty()) {
							...
							//	insert = false;
								existhazopMst=mocService.UpdateHazop(newhazopMst,existhazopMst,HazopKeyId,CreatedBy); 
								CommonFunctions.debugMsg("update");
								savemsg="Data Updated Successfully";					
							}
      ...
      @SuppressWarnings("unchecked")
      private void saveQuestionsdata(HttpServletRequest request,HttpServletResponse response,String MocKeyid)throws Exception{{
   			HttpSession httpsession=request.getSession(false);
   		    ServletOutputStream out=response.getOutputStream();
   		    AdmTlUsermst user=UIUtils.getLoginUser(request);
   		    ...
   		    try{
   		    		...
   		    		if(UIUtils.isValidKeyId(paramJsonArr)){
   		    		  EmployeeList=JSONArray.fromString(paramJsonArr);
   		    		  CommonMessage.debugMsg("EmployeeList"+EmployeeList);
   		    		   EmployeeAddList=(List<MocRfQuestions>)UIUtils.convertJSONArrToList(Employeelink,EmployeeList);		    		   

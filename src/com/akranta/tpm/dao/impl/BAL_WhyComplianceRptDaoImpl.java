...
public class BAL_WhyComplianceRptDaoImpl implements BAL_WhyComplianceRptDao {
	


	private DBActionTemplate dbActionTemplate; 
	
	public BAL_WhyComplianceRptDaoImpl(DBActionTemplate dbActionTemplate) 
	{
	...
	public List<String[]> getAllyycelldrill( String code,CommonFilter commonFilter) throws Exception
	{
		...
		{
			 List<String > paramValues = new ArrayList<String>();
			 System.out.println("Inside  yy DAO Impl cellllll");
			
			 System.out.println("Inside  yy DAO Impl cellllll " +code);
			 String condParms = FilterCondSql.getBDRelatedConditionStr(commonFilter);
			 String commonParams = FilterCondSql.getGridCommonParams(commonFilter); // "ISTOTALCNT="+commonFilter.getViewClick() +";FROMTOROW="+commonFilter.getFromRow() +" AND " + commonFilter.getToRow() +";";
			...
			System.out.println("Inside  yy DAO Impl cellllll" +whyComprowList.size());
			if( commonFilter.getViewClick() == 'Y'){
			String totalCnt = paramValues.get(0); 
			boolean isInteger = Pattern.matches("^\\d*$", totalCnt);
			if(  isInteger ){
	...
	public List<String[]> getAllmonth(CommonFilter commonFilter)throws Exception {
		try
		{
			System.out.println("Inside  yy DAO Implmonthhhhhhhhhhhhhhhhhhhh");
			...
			List<String > paramValues = new ArrayList<String>();
			 ...
			 System.out.println("Inside  yy DAO Impl whyCompmonList" +whyCompmonList.size());
			 if( commonFilter.getViewClick() == 'Y'){
					String totalCnt = paramValues.get(0); 
					boolean isInteger = Pattern.matches("^\\d*$", totalCnt);
				if(  isInteger ){

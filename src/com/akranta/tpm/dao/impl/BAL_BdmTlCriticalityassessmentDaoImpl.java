...
import com.akranta.tpm.dao.BAL_BdmTlCriticalityassessmentDao;
import com.akranta.tpm.dao.sql.BAL_BdmTlCriticalityassessmentSql;
...
public class BAL_BdmTlCriticalityassessmentDaoImpl implements BAL_BdmTlCriticalityassessmentDao {


	private DBActionTemplate dbActionTemplate; 

	public BAL_BdmTlCriticalityassessmentDaoImpl(DBActionTemplate dbActionTemplate)
	{
	...
	public List<String[]> getCriticalityfillgriddata(CommonFilter commonFilter1,String flid,String crytype) throws Exception {
		CommonFunctions.debugMsg("TrainingGrid::::: IP:::");
		...
		CommonFunctions.debugMsg("test to getTrainingGrid.....");	
		...
		List<String[]> dataList =  dbActionTemplate.processFunctionCallsWithColHeaders("PLM_PC_PLANNEDMAINT.PLM_FN_CRITICALITYASSESSMENT", paramValues);
	...
	public List<BAL_BdmTlCriticalityassessment> create(List<BAL_BdmTlCriticalityassessment> newBdmTlCriticalityassessment) throws Exception {

		CommonFunctions.debugMsg("DaoIMPL");
		List<String> sqls = new ArrayList<String>(); /* sqls for execution */ 
		BAL_BdmTlCriticalityassessmentSql bdmTlCriticalityassessmentSql = new BAL_BdmTlCriticalityassessmentSql(); // contains dbtable,field names, Field types and related sqls  of master table
		
		try{
			CommonFunctions.debugMsg("DaoIMPL1");
			if (newBdmTlCriticalityassessment!= null && newBdmTlCriticalityassessment.size() > 0) 
			{CommonFunctions.debugMsg("DaoIMPL2");
				for( BAL_BdmTlCriticalityassessment criticalityassessement :newBdmTlCriticalityassessment)
				{  
					CommonFunctions.debugMsg("DaoIMPL3");   
					...
					//BDM_TL_MCHRANKSKILLHISTORY   First Delete Record based on date for machine
					 ...
					 CommonFunctions.debugMsg(" Inside DaoImpl Outside if "+Eqpid1);
					...
					if(criticalityassessement.getCasmKeyid()== null){
					    ...
					    sqls.add(BAL_BdmTlCriticalityassessmentSql.getInsertSql(bdmTlCriticalityassessmentSql.getCasmDbFields(), criticalityassessement.getSaveArray())); // add insert sql for master table
					    
					    //CommonFunctions.debugMsg(" Inside Dao Impl 1 Totalratings "+criticalityassessement.getCasmTotalratings());
					    CommonFunctions.debugMsg(" Inside Dao Impl 2 Equipment id "+criticalityassessement.getCasmEquipmentid());
					    CommonFunctions.debugMsg(" Inside Dao Impl 3 Flid         "+criticalityassessement.getCasmFlid());
					    ...
					    CommonFunctions.debugMsg(" Inside DaoImpl Inside if "+Eqpid1);
					    //StringBuffer sql1 = BdmTlCriticalityassessmentSql.getmachinerank(Totalratings);
					    StringBuffer sql1 = BAL_BdmTlCriticalityassessmentSql.getCriteria(criticalityassessement.getCasmFlid(), Totalratings , Eqpid,tradeid);
					    CommonFunctions.debugMsg(" Inside DaoImpl to get Machine Rank Values "+sql1);
					    String machRank =dbActionTemplate.getSingleValue(sql1.toString());
					    CommonFunctions.debugMsg(" machRank    "+machRank);
			...
			sqls.add(BdmTlMchrankskillhistorySql.getInsertSql(bdmTlMchrankskillhistorySql.getMrshDbFields(), bdmTlMchrankskillhistory.getSaveArray())); // add insert sql for master table
					...
					{
						sqls.add(BAL_BdmTlCriticalityassessmentSql.getUpdateSql(bdmTlCriticalityassessmentSql.getCasmDbFields(), criticalityassessement.getSaveArray()));
	...
	public List<BAL_BdmTlCriticalityassessment> update(List<BAL_BdmTlCriticalityassessment> newBdmTlCriticalityassessment,List<BAL_BdmTlCriticalityassessment> existBdmTlCriticalityassessment)	throws Exception { 
		
		List<String> sqls = new ArrayList<String>();
		BAL_BdmTlCriticalityassessmentSql bdmTlCriticalityassessmentSql = new BAL_BdmTlCriticalityassessmentSql();
		try {
			if (newBdmTlCriticalityassessment!= null && newBdmTlCriticalityassessment.size() > 0) 
			{
				for( BAL_BdmTlCriticalityassessment Criticalityassessement1 :newBdmTlCriticalityassessment)
				{
					sqls.add(BAL_BdmTlCriticalityassessmentSql.getUpdateSql(bdmTlCriticalityassessmentSql.getCasmDbFields(), Criticalityassessement1.getSaveArray()));
	...
	@Override
	public BAL_BdmTlCriticalityassessment deleteCritical(BAL_BdmTlCriticalityassessment oldBdmTlCriticalityassessment)throws Exception {
		List<String> sqls = new ArrayList<String>();
		BAL_BdmTlCriticalityassessmentSql bdmTlCriticalityassessmentSql = new BAL_BdmTlCriticalityassessmentSql();
		try {
	...
	private ResultSet getControlresplanResultSet(CommonFilter commonFilter) throws Exception {
		...
		paramValues.add(commonParams);
		return dbActionTemplate.dbFunctionCall("PLM_PC_PLANNEDMAINT.PLM_FN_CRITIASSESSMENTMAINGRID", paramValues);
	...
	public List<String[]> getCriAssMainGrid(CommonFilter commonFilter) throws Exception {
		...
		CommonFunctions.debugMsg("test to............");
		
		paramValues.add(condParms);
		paramValues.add(commonParams); 
		
		List<String[]> dataList =  dbActionTemplate.processFunctionCallsWithColHeaders("PLM_PC_PLANNEDMAINT.PLM_FN_CRITIASSESSMENTMAINGRID", paramValues);
	...
	public String DeleteCriteriaList(String criteriaDeleteList) {
		...
		try{
			if(UIUtils.isValidKeyId(criteriaDeleteList)){	
				...
				CommonFunctions.debugMsg("criteriaDeleteList "+criteriaDeleteList);
	...
	public List<String[]> getCriticalassmntcritria( String flid , String equm, String total,String trade ) throws Exception {
		 StringBuffer sql1 = BAL_BdmTlCriticalityassessmentSql.getCriteria( flid , total , equm ,trade);
		 CommonFunctions.debugMsg(" Inside DaoImpl to get Machine Rank Values "+sql1);
	...
	public String getCriticalassmntremarks(String flid, String equm)
		...
		CommonFunctions.debugMsg(" Inside DaoImpl to get Machine remarks "+sql);			
	...
	public Workbook getcriticalReporcExcel(JSONObject colmodel, String format,
		...
		ResultSet rs = null;
		 try
		 {
				...
				CommonFunctions.debugMsg("daoimpl22222");*/
				return excelUtils.writeToExcel(rs,format,2,0,0 );
	...
	private ResultSet getcriticalReporcExcel(CommonFilter commonFilter) throws Exception {
		...
		CommonFunctions.debugMsg("result");

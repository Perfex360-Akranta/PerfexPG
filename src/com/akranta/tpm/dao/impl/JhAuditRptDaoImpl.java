...
public class JhAuditRptDaoImpl implements JhAuditRptDao {

private DBActionTemplate dbActionTemplate;
FunctionCallApi fnCallApi;
	
	public JhAuditRptDaoImpl(DBActionTemplate dbActionTemplate) 
	{
	...
	public List<String []> getJhAuditRpt(CommonFilter commonFilter) throws Exception
	{
		try
		{
			...
			CommonMessage.debugMsg(" maintype Dao Impl :: "+maintype);
			...
			if(UIUtils.isValidKeyId(commonFilter.getType())|| UIUtils.isValidKeyId(commonFilter.getMaintMode())){
				 //dataList =  dbActionTemplate.processFunctionCalls("GEN_FN_JHAUDITACTIONPLANSCORE", paramValues);
				dataList = fnCallApi.callFunction("GEN_FN_JHAUDITACTIONPLANSCORE_SB", paramValues,3,true);
				
			}else{
			     dataList =  dbActionTemplate.processFunctionCalls("JHN_FN_JHAUDIT", paramValues);
			}
	...
	public Map<Integer, List<String[]>> getAllJHAuditRptExl(String rowId,String format) throws Exception {
		try
		{
			...
			Map<Integer, List<String[]>> whyReport = dbActionTemplate.processDbFunCallMultCursor("JHN_FN_JHAUDITREPORTEXL", paramValues,2);
	...
	private ResultSet getJhAuditReport(CommonFilter commonFilter) throws Exception
	{
		...
		ResultSet rs = dbActionTemplate.dbFunctionCall("JHN_PC_JHN_FN_JHAUDIT", paramValues);
		CommonMessage.debugMsg("rs value.....");
		return rs;
	}

	/*
	 * @Override public Workbook JhAuditActionReportExportExcel(CommonFilter
	 * commonFilter, JSONObject tblJSONObj, String format) throws Exception { //
	 * TODO Auto-generated method stub ResultSet rs = null; try{
	 * 
	 ...
	 * 
	 * 
	 * return excelUtils.writeToExcel(rs,format,2,1,0 );//sriram
	 * 
	...
	public Workbook JhAuditActionReportExportExcel(CommonFilter commonFilter,
		...
		ResultSet rs = null;
		   try{
			...
			System.out.println("tblJSONObj EXCEL VAL::::::::"+ tblJSONObj);
			

	         
			return excelUtils.writeToExcel(rs,format,3,1,0 );//sriram //CHANGE HERE 5TH May - rowstart 2->3
	...
	private ResultSet getJhAuditActionReport(CommonFilter commonFilter) throws Exception {
		...
		return dbActionTemplate.NewdbFunctionCall2("GEN_FN_JHAUDITACTIONPLANSCORE_SB", paramValues);//Change here 5th May _sb
		
}

/*
 * private ResultSet getJhAuditActionReport(CommonFilter commonFilter) throws
 * Exception { // TODO Auto-generated method stub List<String> paramValues = new
 * ArrayList<String>(); String condParms =
 * FilterCondSql.getJHCLITRelatedCondStr(commonFilter); //
 * OPLRelatedCondSql(commonFilter) String commonParams =
 ...
 * +="MAINTYPE="+commonFilter.getMaintMode()+";EXCELIDEN=EXCEL"; }
 * 
 * 
 * paramValues.add(condParms); paramValues.add(commonParams);
 * 
 * return dbActionTemplate.NewdbFunctionCall2("GEN_FN_JHAUDITACTIONPLANSCORE",
 * paramValues);
 * 
 * }
 */
	...
	public List<String[]> getAllAuditScoreGraph(CommonFilter commonFilter) throws Exception {
		try
		{
			...
			//List<String[]> impVsCompList = dbActionTemplate.processFunctionCallsWithColHeaders("JHN_FN_AUDITSCOREGRAPH", paramValues);
			List<String[]> impVsCompList = fnCallApi.callFunction("JHN_FN_AUDITSCOREGRAPH_SB", paramValues,3,false);
			if( commonFilter.getViewClick() == 'Y'){
				String totalCnt = paramValues.get(0); 
				boolean isInteger = Pattern.matches("^\\d*$", totalCnt);
				if(  isInteger ){
	...
	public Workbook jhAuditGraphExportExcel(CommonFilter commonFilter,JSONObject colModel, String rptFormat) throws Exception {
		ResultSet rs = null;
		   try{
			...
			condFormats.add(condFormat);
			excelUtils.setCondFormats(condFormats);
			return excelUtils.writeToExcel(rs,rptFormat, 2,0,0 );//sriram
	...
	private ResultSet getIncedentVsCompleteResultSet(CommonFilter commonFilter) throws Exception
	{
		...
		return dbActionTemplate.NewdbFunctionCall2("JHN_FN_AUDITSCOREGRAPH", paramValues);
	}

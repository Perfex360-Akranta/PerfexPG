...
public class BAL_PlmTlStandardsSql {
	...
	public enum tableFldConstants {
		keyid, date, factoryid, sectionid, cellid, machineid, assemblyid, subassemblyid, eqpgroupid, source, supplierid,
		tradeid, frequency, frequencyunit, uomid, howmethod, duration, activitytype, activitysubtype, bomid, location,
		activity, standard, machinecondition, planconfigstatus, issparesreq, istoolsreq, refdoctype, refdocno,
		preparedbyid, formatno, effectivedate, wogenflag, phenomenaid, causeid, routenumber, includeinshutdownmaint,
		groupno, resultifnotdone, correctiveaction, issftpermitreq, safetyinstruction, inactivateddate, monthweekno,
		relatedTo, mouldid, locationid, flid, elementid, maxvalue, minvalue, target, tempfield8, tempfield9,
	...
	public static String getsprData(String pmsdkeyid) {
		// TODO Auto-generated method stub
		String sql = "SELECT  SPRM_PARTNO,SPRM_PARTNAME,SPRM_MAKE,SPRM_MODEL,PSPD_QUANTITY FROM " + TBL_PLM_TL_SPAREDTL
				+ "," + TBL_GEN_TL_SPARESMST + "," + TBL_BAL_PLM_TL_STANDARDS
	...
	public static String getSparesPickupTbl() {
		...
		String sql = "Select PSPD_SPAREID, SPRM_PARTNO, SPRM_PARTNAME ,SPRM_MAKE,SPRM_MODEL, "
				+ " PSPD_QUANTITY, PSPD_KEYID  from " + TableNames.TBL_GEN_TL_SPARESMST + ","
				+ TableNames.TBL_BAL_PLM_TL_SPAREDTL + " where SPRM_KEYID = PSPD_SPAREID and PSPD_STANDARDID = ? ";
	...
	public static String gettooldata(String toolpmsdid) {
		...
		String sql = "SELECT PTLD_TOOLID, TOLM_NAME " + "FROM " + TBL_PLM_TL_TOOLSDTL + " t " + "JOIN "
				+ TBL_GEN_TL_TOOLSMST + " m " + "ON t.PTLD_TOOLID = m.TOLM_KEYID " + "WHERE t.PTLD_STANDARDID = '"
				+ toolpmsdid + "'";
	...
	public static String updateDocUpdatesdatasql(String dKeyId, String pmsdKeyId) {
		// TODO Auto-generated method stub
		String sql = "UPDATE " + TBL_GEN_TL_DOCUPDATES + " SET DCUP_DETAILID = '" + pmsdKeyId + "' Where DCUP_KEYID= '"
				+ dKeyId + "'";
	...
	/*
	 * public static String getCBMTbl(String pmStandardId) { // TODO Auto-generated
	 * method stub //CMDT_CBMCONDITION replaced with ZONM_CORRECTIVECONDITION String
	 * cbmSql = null; //if(UIUtils.isValidKeyId(pmStandardId)){ cbmSql =
	 * "SELECT   ZONM_KEYID , CMDT_INSPECTIONID, REPLACE(REPLACE(ZONM_NAME,'<*',''),'*>',''), CMDT_ZONECOLOR, CMDT_LOWERLIMIT,  CMDT_UPPERLIMIT, CMDT_DESIRABLEREADING,  "
	 * +
	 ...
	 * + " FROM "+TableNames.TBL_PLM_TL_CBMSTDCADTL+","+TableNames.
	 * TBL_PLM_TL_ZONEMST+" WHERE  CMDT_PMSTANDARDID(+) = ? AND CMDT_ZONEID(+) = ZONM_KEYID  order by ZONM_CODE "
	 * ;// DECODE(ZONM_CODE,'R',3,'Y',2,'G',1)"; } else{ cbmSql =
	 * "SELECT  ZONM_KEYID, ZONM_COLOR, ZONM_NAME, ZONM_CODE,'','','',  ZONM_CORRECTIVEACTION, ZONM_CORRECTIVECONDITION"
	 * + " FROM "+TableNames.
	 * TBL_PLM_TL_ZONEMST+" order by DECODE(ZONM_CODE,'R',3,'Y',2,'G',1)"; } return
	 * cbmSql; }
	 */
	public static String getCBMTbl(String pmStandardId) {
	    ...
	    cbmSql = "SELECT " +
	            "ZONM_KEYID, " +
	            "CMDT_INSPECTIONID, " +
	            "REPLACE(REPLACE(ZONM_NAME, '<*', ''), '*>', ''), " +
	            "CMDT_ZONECOLOR, " +
	            "CMDT_LOWERLIMIT, " +
	            "CMDT_UPPERLIMIT, " +
	            "CMDT_DESIRABLEREADING, " +
	            "REPLACE(REPLACE(CMDT_CORRECTIVEACTION, '<*', ''), '*>', ''), " +
	            "ZONM_CORRECTIVECONDITION, " +
	            "'' AS COLUMN10, " +
	            "REPLACE(REPLACE(REPLACE(CMDT_MEASURINGMETHOD, '<*', ''), '*>', ''), '{}', ''), " +
	            "CMDT_PMSTANDARDID, " +
	            "CMDT_UOMID, " +
	            "CMDT_KEYID " +
	            "FROM " + TableNames.TBL_BAL_PLM_TL_ZONEMST + " Z " +
	            "LEFT JOIN " + TableNames.TBL_BAL_PLM_TL_CBMSTDCADTL + " C " +
	            "ON C.CMDT_ZONEID = Z.ZONM_KEYID " +
	            "AND C.CMDT_PMSTANDARDID = ? " +
	...
	public static String getMethodTaskList(String machineId) {
		return "Select MTSK_OPERATION,MTSK_CHECKINGTOOL,MTSK_IDEALCONDITION,MTSK_TYPEOFCHECK,MTSK_ACTUALCONDITION from plm_tl_methodtasklist where MTSK_MACHINEID = '"
				+ machineId + "'";
	}
